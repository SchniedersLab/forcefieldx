...
// Unit:    Class edu.rit.pj.test.Test22
//
...
/**
 * Class Test22 is a unit test main program for classes {@linkplain
 * edu.rit.pj.WorkerTeam WorkerTeam}, {@linkplain edu.rit.pj.WorkerRegion
 * WorkerRegion}, and {@linkplain edu.rit.pj.WorkerLongForLoop
 * WorkerLongForLoop}. A worker for loop iterates over a given range of indexes.
 * Each iteration prints the loop index and the worker index executing that loop
 * index.
 * <P>
 * Usage: java [ -Dpj.np=<I>K</I> ] [ -Dpj.schedule=<I>schedule</I> ]
 * edu.rit.pj.test.Test22 <I>lb</I> <I>ub</I> <I>stride</I>
	...
	public static void main
		...
		new WorkerTeam().execute (new WorkerRegion()
			...
			public void run() throws Exception
				...
				if (stride == 1)
					{
					execute (lb, ub, new WorkerLongForLoop()
						{
					...
					{
					execute (lb, ub, stride, new WorkerLongStrideForLoop()
						{
