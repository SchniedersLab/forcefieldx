...
public class FFXExec implements Runnable {
  ...
  /**
   * Executes the native call to "System()" and notifies the ResultPanel upon completion. This
   * should only be called indirectly by Thread.Start()
   */
  public void run() {
    setEnv();
    if (args == null
    ...
    if (returnValue < 0) {
      Logger.getLogger("ffx")
          .warning("The following job exited with a failure status: " + returnValue + "\n" + args);
    }
    // Open any created file and display the log.
    if (mainPanel != null) {
      if (newFile != null) {
        String[] labels = args.split(" +");
        String command = labels[0].toUpperCase() + " on " + system.getFile().getName();
        if (openOnto) {
  ...
  public String toString() {
    ToStringBuilder toStringBuilder =
        new ToStringBuilder(this).append(path).append(classpath).append(ld_library_path);
   ...
   * @return int
   */
  private native int nativeExec(String argv, String dir, String path, String classpath, String jre);
}
