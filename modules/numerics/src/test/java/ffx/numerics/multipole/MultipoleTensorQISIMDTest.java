...
public class MultipoleTensorQISIMDTest {
  ...
  public MultipoleTensorQISIMDTest(String info, int order, Operator operator) {
    this.info = info;
    this.order = order;
    ...
    rVectors = new DoubleVector[]{xVector, yVector, zVector};
  ...
  public void permanentMultipoleEnergyAndGradTest() {
    ...
    MultipoleTensorSIMD multipoleTensor = new CoulombTensorQISIMD(order);
    ...
    QIFrameSIMD qiFrame = new QIFrameSIMD();
    qiFrame.setAndRotate(rVectors, mI, mK);
    ...
    multipoleTensor.setR(xVector, yVector, zVector);
    multipoleTensor.generateTensor();
    mI = new PolarizableMultipoleSIMD(qI, uI, uI);
    mK = new PolarizableMultipoleSIMD(qK, uK, uK);
    rVectors = new DoubleVector[]{xVector, yVector, zVector};
    qiFrame.setAndRotate(rVectors, mI, mK);
  ...
  public void polarizationEnergyAndGradientTest() {
    ...
    for (int i = 0; i < DoubleVector.SPECIES_PREFERRED.length(); i++) {
      assertEquals(info + " Polarization Energy", polarizationEnergyCoulomb, eArray[i], tolerance);
