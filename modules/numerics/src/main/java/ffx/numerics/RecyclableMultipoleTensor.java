...
import static ffx.numerics.RecyclableMultipoleTensor.OPERATOR.COULOMB;
import static ffx.numerics.RecyclableMultipoleTensor.OPERATOR.SCREENED_COULOMB;
import static ffx.numerics.RecyclableMultipoleTensor.OPERATOR.THOLE_FIELD;
 ...
 * @since 1.0
 */
public class RecyclableMultipoleTensor {
    ...
    private double z;
    /**
     * Original (unrotated) distances from previous call.
     * Determines tensor re-use and allows short-circuiting setR().
     */
    private double[] rorig = null;
    ...
    public boolean recycleTensors = Boolean.valueOf(System.getProperty("sys.reuseTensors","false"));
    private static final double[] noDipole = new double[]{0.0,0.0,0.0};
     ...
     */
    public RecyclableMultipoleTensor(OPERATOR operator, COORDINATES coordinates, int order, double aewald) {
        assert (order > 0);
        o1 = order + 1;
        il = o1;
        im = il * o1;
    ...
    public void resetRecycledCount() {
        tensorsRecycled = 0;
    }
    ...
    public void setTholeDamping(double pgamma, double aiak) {
        this.pgamma = pgamma;   // min(thole[i], thole[k])
        this.aiak = aiak;		// ipdamp[i] * ipdamp[k]
    }
    ...
    /**
     * @param r distance
     * @return whether damping should be applied
     */
    public boolean checkAndSetDamping(double[] r, double tholei, double tholek, double ipdampi, double ipdampk) {
        final double rdamp = r(r) * ipdampi * ipdampk;
        final double damp = -min(tholei,tholek) * rdamp*rdamp*rdamp;
        if (damp > -50.0) {
     ...
     */
    private void generateTensor(OPERATOR operator, double[] r, double lambdaFunction, double[] Qi, double[] Qk,
            double[] ui, double[] uiCR, double[] uk, double[] ukCR, double damp, double aiak) {
        boolean operatorChange = this.operator != operator;
        if (operatorChange)
            setOperator(operator);
        if (operator == THOLE_FIELD)
            setTholeDamping(damp, aiak);
        boolean distanceChanged = setR(r, lambdaFunction);
        setMultipoles(Qi, Qk);
        setDipoles(ui, uiCR, uk, ukCR);
        if (distanceChanged || operatorChange || r2 == 0.0)
            generateTensor();
        else tensorsRecycled++;
    }
    
    private void generateTensor(double[] r, double lambdaFunction, double[] Qi, double[] Qk,
            double[] ui, double[] uiCR, double[] uk, double[] ukCR, double damp, double aiak) {
        if (operator == THOLE_FIELD)
            setTholeDamping(damp, aiak);
        else
    ...
    public void generateTensor(double[] r, double lambdaFunction, double[] Qi, double[] Qk) {
        unsetDamping();
        boolean rchanged = setR(r, lambdaFunction);
        setMultipoles(Qi, Qk);
        unsetDipoles();
        if (rchanged || !recycleTensors)
            generateTensor();
        else tensorsRecycled++;
    }
    /**
    ...
    public void permScreened(double r[], double lambdaFunction, double[] Qi, double[] Qk) {
        boolean operatorChange = operator != SCREENED_COULOMB;
        if (operatorChange)
            setOperator(SCREENED_COULOMB);
        unsetDamping();
        boolean distanceChanged = setR(r, lambdaFunction);
        setMultipoles(Qi, Qk);
        unsetDipoles();
        if (!recycleTensors || distanceChanged || operatorChange || r2 == 0.0)
            generateTensor();
        else tensorsRecycled++;
    }
    /**
    ...
    public void permCoulomb(double r[], double lambdaFunction, double[] Qi, double[] Qk) {
        boolean operatorChange = operator != COULOMB;
        if (operatorChange)
            setOperator(COULOMB);
        unsetDamping();
        boolean distanceChanged = setR(r, lambdaFunction);
    ...
    public void tholeField(double[] r, double[] Qi, double[] Qk,
            double[] ui, double[] uiCR, double[] uk, double[] ukCR, double minThole, double ipdampProduct) {
        boolean operatorChange = operator != THOLE_FIELD;
        if (operatorChange)
            setOperator(THOLE_FIELD);
        setTholeDamping(minThole, ipdampProduct);
        boolean distanceChanged = setR(r);
        setMultipoles(Qi, Qk);
        setDipoles(ui, uiCR, uk, ukCR);
        if (!recycleTensors || distanceChanged || operatorChange || r2 == 0.0)
    ...
    /**
     * For Thole tensors whose damping parameters have already been set via checkAndSetDamping().
     * @see MultipoleTensor#generateTensor(double[], double, double[], double[],
     * double[], double[], double[], double[], double, double)
     */
    public void tholeField(double[] r, double[] Qi, double[] Qk,
     ...
     */
    boolean setR(double r[], double lambdaFunction) {
        switch (coordinates) {
            case QI:
                return setR_QI(r, lambdaFunction);
            default:
            case GLOBAL:
                if (r[0] == x && r[1] == y && r[2]+lambdaFunction == z) {
                    return true;
                }
                x = r[0];
                y = r[1];
    ...
    private boolean setR_QI(double r[]) {
        return setR_QI(r, 0.0);
    }

    /**
     * @return Whether anything changed as a result.
     */
    private boolean setR_QI(double r[], double lambdaFunction) {
        if (rorig != null && r[0] == rorig[0] && r[1] == rorig[1] && r[2] == rorig[2] && lambdaFunction == rorig[3]) {
            return true;
        }
        if (rorig == null) rorig = new double[4];
        rorig[0] = r[0];
        rorig[1] = r[1];
        rorig[2] = r[2];
        rorig[3] = lambdaFunction;
        x = 0.0;
        y = 0.0;
     ...
     */
    public double indiFieldsDotKqi(double scaleDipoleCR, double scaleDipole) {
     ...
     */
    public double indkFieldsDotIqi(double scaleDipoleCR, double scaleDipole) {
...
//	private double dotMultipoleI() {
    ...
    public static void main(String args[]) {
        ...
        RecyclableMultipoleTensor globalTensor = new RecyclableMultipoleTensor(
                OPERATOR.SCREENED_COULOMB, COORDINATES.GLOBAL, order, 1e-6);
        RecyclableMultipoleTensor qiTensor = new RecyclableMultipoleTensor(
                OPERATOR.SCREENED_COULOMB, COORDINATES.QI, order, 1e-6);
        ...
        if (testTimings) {
            ...
            for (int j = 0; j < cycles; j++) {
                ...
                for (int i = 0; i < n2; i++) {
                    ...
                    r[2] = Math.random();
                    globalTensor.setR(r);
                    globalTensor.setMultipoles(Qi, Qk);
                    double e = globalTensor.multipoleEnergy(Fi, Ti, Tk);
                }
                ...
                for (int i = 0; i < n2; i++) {
                    ...
                    r[2] = Math.random();
                    qiTensor.setR(r);
                    qiTensor.setMultipoles(Qi, Qk);
                    double e = qiTensor.multipoleEnergy(Fi, Ti, Tk);
                }
        ...
        if (testValues) {
            ...
            for (int i = 0; i < iters; i++) {
                ...
                r[1] = Math.random();
                r[2] = Math.random();
                RecyclableMultipoleTensor tensor = new RecyclableMultipoleTensor(OPERATOR.SCREENED_COULOMB, COORDINATES.GLOBAL, order, 1e-6);
                tensor.permScreened(r, buffer, Qi, Qk);
                double eGlob = tensor.multipoleEnergy(Fi, Ti, Tk);
                double deGlob = tensor.getdEdZbuff();
                RecyclableMultipoleTensor tensorQi = new RecyclableMultipoleTensor(OPERATOR.SCREENED_COULOMB, COORDINATES.QI, order, 1e-6);
                tensorQi.permScreened(r, buffer, Qi, Qk);
        ...
        if (testDeriv) {
            ...
            tensor = new RecyclableMultipoleTensor(OPERATOR.SCREENED_COULOMB, COORDINATES.QI, order, 1e-6);
            tensor.setR(r, buffer);
            tensor.setMultipoles(Qi, Qk);
            tensor.order5QI();
            double eQI = tensor.multipoleEnergy(Fi, Ti, Tk);
    ...
    String printGlobalToQI() {
        double[] g2qi = new double[]{r00,r01,r02,r10,r11,r12,r20,r21,r22};
        String ret = " Global->QI matrix: ";
        for (int i = 0; i < g2qi.length; i++) {
            ret += format("%6.3f",g2qi[i]);
        }
        return ret;
    }

    String printQIToGlobal() {
        double[] qi2g = new double[]{ir00,ir01,ir02,ir10,ir11,ir12,ir20,ir21,ir22};
        String ret = " QI->Global matrix: ";
        for (int i = 0; i < qi2g.length; i++) {
            ret += format("%6.3f",qi2g[i]);
        }
        return ret;
    }
