 ...
 *
 * <p>Adapted from CERN's cern.jet.math.tdouble package as included with ParallelColt:
...
public class ModifiedBessel {
  ...
  /**
   * Chebyshev coefficients for exp(-x) i1(x) / x in the interval [0,8]. lim(x->0){ exp(-x) i1(x) /
   * x } = 1/2.
   */
  ...
  /**
   * Modified zero-order Bessel function. The function is defined as i0(x) = J0( ix ). The range is
   * partitioned into the two intervals [0,8] and (8, infinity). Chebyshev polynomial expansions are
  ...
  /**
   * Modified 1st-order ModifiedBessel function. The function is defined as i1(x) = -i j1( ix ). The
   ...
   *
   * <p>NOTE: Argument x must first be transformed to the interval (-1,1).
   *
   * <p>NOTE: Coefficients are in reverse; zero-order term is last.
   *
  ...
  /**
   * Returns Double.MAX_VALUE in place of Double.POSITIVE_INFINITY; Returns Double.MIN_VALUE in
   * place of Double.NEGATIVE_INFINITY
   *
   * @param x input parameter
   * @return exp(x)
   */
