 ...
 *
 * <p>The decimal accuracy compared to Math.sqrt is: <br>
 * within 1.0e-13 for input values from 1.0e-2 to 1.0e3. <br> within 1.0e-12 for input values from
 * 1.0e-4 to 1.0e4. <br> within 1.0e-11 for input values from 1.0e-5 to 1.0e5. <br> within 1.0e-10
 * for input values from 1.0e-8 to 1.0e8.
 *
 * <p>The "main" method performance test shows that the software implementation is slower than
 * simply calling Math.sqrt.
 *
...
public class SquareRoot {
  ...
  /**
   * Compute the inverse square root <code>1.0/sqrt(x2)</code> of the input value x2 using a look-up
   * table and two iterations of Newton's method for finding roots of an equation.
   *
  ...
  public static double isqrt(double x2) {
    ...
    /* Return here is 5 Flops */
    // return y;

    /* Second iteration of Newton's method. */
    /* Return here is 10 Flops */
    return half * y * (three - ((x2 * y) * y));
  }
  ...
  public static void main(String[] args) {
    for (int j = 0; j < 10; j++) {
      ...
      System.out.printf(" Software sum was    %16.8f in %16.8f seconds.%n", sum, 1.0e-9 * time);
      ...
      time += System.nanoTime();
      System.out.printf(" 1/Math.sqrt sum was %16.8f in %16.8f seconds.%n", sum, 1.0e-9 * time);
    }
  }

  /**
   * Compute the square root of the input value x. The method calls <code>isqrt</code> and then
   * multiples by the input <code>sqrt(x2) = x2 * isqrt(x2)</code>.
   *
  ...
  /**
   * The Mask 0x7f800000 selects the Exponent Bits 30-23, which are then shifted 23 places to the
   * right.
   *
  ...
  /**
   * The Mask 0x00800000 selects the least significant Exponent bit and the Mask 0x007fffff selects
   * the Mantissa Bits 22-0, which are then shifted 12 places to the right. The shift of 12 comes
   * from the Exponent shift (23) minus the significant part of the mantissa (11).
   *
   * @param val input parameter
