...
public class QuickHull3D {
  ...
  protected void createInitialSimplex() throws IllegalArgumentException {
    ...
    if (logger.isLoggable(Level.FINE)) {
      ...
      logger.fine(vtx[0].index + ": " + vtx[0].pnt);
      logger.fine(vtx[1].index + ": " + vtx[1].pnt);
      logger.fine(vtx[2].index + ": " + vtx[2].pnt);
      logger.fine(vtx[3].index + ": " + vtx[3].pnt);
    }
  ...
  protected void resolveUnclaimedPoints(FaceList newFaces) {
    ...
    for (Vertex vtx = vtxNext; vtx != null; vtx = vtxNext) {
      ...
      if (maxFace != null) {
        ...
        if (logger.isLoggable(Level.FINE) && vtx.index == findIndex) {
          logger.fine(findIndex + " CLAIMED BY " + maxFace.getVertexString());
        }
      } else {
        if (logger.isLoggable(Level.FINE) && vtx.index == findIndex) {
          logger.fine(findIndex + " DISCARDED");
        }
      }
    }
  }
