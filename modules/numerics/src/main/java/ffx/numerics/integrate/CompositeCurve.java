...
public class CompositeCurve extends FunctionDataCurve {
    ...
    public CompositeCurve(List<FunctionDataCurve> componentCurves, List<Double> coefficients) {
        ...
        boolean isInvalid = stream(curves).anyMatch((FunctionDataCurve c) -> {
            if (lb != c.lowerBound()) {
                return true;
            }
            if (ub != c.upperBound()) {
    ...
    public String toString() {
        StringBuilder sb = new StringBuilder(format("Composite curve with %d points from lower bound %9.3g and upper bound %9.3g", points.length, lb, ub));
        if (halfWidthEnd) {
