...
/**
 * This class performs rescoring on a provided list of structure files. Rescoring can be based on
 * energy evaluations, minimization, x-ray minimization, or real-space minimization.
 *
 * @author Jacob Litman
 * @since 1.0
 */
public class Rescore {
  ...
  public File[] rescore(File[] modelFiles) {
    ...
    for (int i = 0; i < numAccepted; i++) {
      ...
      logger.info(
          String.format(" Accepted: %s at %10.6f kcal/mol", relPath, energies[i].getDoubleValue()));
    }
    for (int i = numAccepted; i < numFiles; i++) {
      ...
      String relPath = pwdPath.relativize(pathi).toString();
      logger.info(
          String.format(" Rejected: %s at %10.6f kcal/mol", relPath, energies[i].getDoubleValue()));
    }
    ...
    try (BufferedWriter bw = new BufferedWriter(new FileWriter(resultsFile, true))) {
      ...
      logger.info(
          String.format(
              " Printing accepted files to xray.groovy.test.rescore file %s", rscFileName));

      if (acceptThreshold > 0.0) {
        String message =
            String.format(
                "Minimum potential energy: %f, threshold = %6.4f", minEnergy, acceptThreshold);
        bw.write(message);
        message = " " + message + "\n";
        logger.info(message);
      } else {
        ...
        String message =
            String.format(
                "Minimum potential energy: %f, maximum accepted energy %f", minEnergy, maxEnergy);
        bw.write(message);
        message = " " + message + "\n";
        logger.info(message);
      }
      ...
      for (int i = 0; i < numAccepted; i++) {
        ...
        logger.info(
            String.format(
                " Accepted file %d energy %9.3f < %9.3f kcal/mol",
                (i + 1), thisEnergy, minEnergy + acceptThreshold));
        logger.info(String.format(" %s", relPath));
        try {
          bw.write(
              String.format("Accepted file: %s rank %d energy %f\n", relPath, (i + 1), thisEnergy));
          if (printModels) {
            bw.newLine();
          ...
          logger.warning(
              String.format(
                  " File %s had exception printing to xray.groovy.test.rescore file %s",
                  relPath, ex.toString()));
        }
      }
      ...
      if (includeRejected) {
        for (int i = numAccepted; i < numFiles; i++) {
          ...
          message =
              String.format(
                  "Non-accepted file: %s rank %d energy %f", relPath, (i + 1), thisEnergy);
          logger.info(String.format(" %s", message));
          try {
            bw.write(message);
            bw.newLine();
      ...
      logger.warning(
          String.format(" Exception in writing xray.groovy.test.rescore file: %s", ex.toString()));
    }
    return rescoredFiles;
  }

  /**
   * Launch the rescoring algorithm on provided files. Assumes it has been given valid files to be
   * run on; use CoordinateFileFilter.acceptDeep(File file) before sending files to this method.
   *
  ...
  /**
   * Setter for the field <code>acceptThreshold</code>.
   *
  ...
  /**
   * Setter for the field <code>diffractionFiles</code>.
   *
  ...
  /**
   * Setter for the field <code>fileSuffix</code>.
   *
  ...
  /**
   * Setter for the field <code>includeRejected</code>.
   *
  ...
  /**
   * Setter for the field <code>mapFiles</code>.
   *
  ...
  /**
   * Setter for the field <code>numToAccept</code>.
   *
  ...
  /**
   * Setter for the field <code>printModels</code>.
   *
  ...
  /**
   * Setter for the field <code>resultsFile</code>.
   *
  ...
  private File rescoreSingle(
      File modelFile, RescoreStrategy rscType, DoubleIndexPair[] energies, int i) {
      ...
      switch (rscType) {
        ...
        case XRAY_MIN:
          ...
          RefinementMinimize refinementMinimize =
              new RefinementMinimize(diffractionData, refinementMode);
          if (eps < 0.0) {
          ...
          if (diffractionFile != null) {
            try {
              ...
              // This should never occur, because diffractionFiles should be of a List type
              // supporting remove(object).
              diffractionFiles = new ArrayList<>();
            }
          }
          break;
        case RS_MIN:
          ...
          RealSpaceData realspaceData =
              new RealSpaceData(
                  assembly, properties, new ParallelTeam(), mapFiles.toArray(new RealSpaceFile[0]));
          utils.energy(assembly);
