...
def cli = new CliBuilder(
    usage: ' ffxc loopBuilder [options] <pdbFile> <realSpaceMapFile | diffractionFile>');
...
if (arguments.size() > 1) {
  ...
  if (FilenameUtils.isExtension(dataFileName, "map")) {
    ...
    String mapFileName =
        String.format("%s_ffx_%d", FilenameUtils.removeExtension(dataFileName), ++nDiffractionData);
...
if (runOST) {
  ...
  RefinementEnergy refinementEnergy = new RefinementEnergy(realSpaceData,
      RefinementMode.COORDINATES, null);
