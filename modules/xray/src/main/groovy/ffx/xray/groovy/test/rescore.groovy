...
def cli = new CliBuilder(usage: ' ffxc xray.test.xray.groovy.test.rescore [options] <modelfilename> [datafilename]');
...
cli.pm(longOpt: 'printModels', args: 1, argName: 'false', 'Prints models to xray.groovy.test.rescore file if true; else only prints file names');
...
cli.dr(longOpt: 'directory', args: 1, argName: 'directory', 'If set, xray.groovy.test.rescore files are written to this directory: is made if it does not exist.');
...
if (options.h || arguments == null || arguments.size() < 1) {
    logger.info(" modelfilename can either be a file with a list of files to xray.groovy.test.rescore, or a directory/folder of files to xray.groovy.test.rescore.");
...
if (resultsFile.exists()) {
    ...
    if (resultsFile.exists()) {
        logger.warning(String.format(" Versioning failed: appending xray.groovy.test.rescore file to end of file %s", resultsFile.getName()));
        versioned = false;
    }
}
