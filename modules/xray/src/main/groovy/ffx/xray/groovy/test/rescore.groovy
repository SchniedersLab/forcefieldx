...
def cli = new CliBuilder(
    usage: ' ffxc xray.test.xray.groovy.test.rescore [options] <modelfilename> [datafilename]');
cli.h(longOpt: 'help', 'Print this help message.');
cli.m(longOpt: 'minimize', args: 1, argName: '1',
    'Rescore on energy evaluation (1), minimization (2), minimization with X-ray target (3), or minimization with real-space target (4).');
cli.e(longOpt: 'eps', args: 1, argName: '-1.0',
    'RMS gradient convergence criteria (negative: automatically determine based on refinement type)');
cli.xd(longOpt: 'xrayData', args: 3, valueSeparator: ',', argName: 'data,1.0,false',
    'specify input xray data filename or filenames (or simply provide the datafilename argument after the PDB file), weight applied to the data (wA) and if the data is from a neutron experiment.');
cli.rd(longOpt: 'realspaceData', args: 2, valueSeparator: ',', argName: 'data,1.0',
    'specify input map data filename or filenames (or simply provide the datafilename argument after the PDB file) and weight applied to the data.');
cli.i(longOpt: 'maxiter', args: 1, argName: '1000',
    'maximum number of allowed refinement iterations');
...
cli.r(longOpt: 'resultsFile', args: 1, argName: 'ffx_rescore.txt',
    'File to output accepted models and scores to.');
cli.pm(longOpt: 'printModels', args: 1, argName: 'false',
    'Prints models to xray.groovy.test.rescore file if true; else only prints file names');
cli.t(longOpt: 'acceptThreshold', args: 1, argName: '0.0',
    'Accepts models within this distance (kcal/mol) of best structure; over-rides number to accept if >0.0');
cli.n(longOpt: 'numberToAccept', args: 1, argName: '10',
    'Accepts this many of the best structures.');
cli.dr(longOpt: 'directory', args: 1, argName: 'directory',
    'If set, xray.groovy.test.rescore files are written to this directory: is made if it does not exist.');
cli.ir(longOpt: 'includeRejected', args: 1, argName: 'true',
    'If set, includes names of files not accepted.');
...
if (options.h || arguments == null || arguments.size() < 1) {
  logger.info(
      " modelfilename can either be a file with a list of files to xray.groovy.test.rescore, or a directory/folder of files to xray.groovy.test.rescore.");
  logger.info(
      " To minimize multiple structures with different data/map files, enter only a directory containing models and identically-named data/map files (ex: abc.pdb and abc.mtz).\n");
  return cli.usage();
}
...
try {
  ...
  logger.
      severe(" FFX could not establish path to current working directory. FFX will not continue.");
}
...
if (options.m) {
  ...
  switch (refineMode) {
    case 1:
      ...
      if (arguments.size() != 1) {
        logger.warning(String.format(" Improper number of arguments (must be 1) for -m mode %d",
            refineMode));
        return cli.usage();
      }
      break;
    ...
    case 3:
      ...
      if (arguments.size() > 2) {
        logger.warning(String.format(
            " Improper number of arguments (must be 1 or 2) for -m mode %d", refineMode));
        return cli.usage();
      }
      break;
    case 4:
...
try {
    ...
    try {
      ...
      while (line != null) {
        ...
        if (!model.exists()) {
          ...
          logger.info(String.format(" File %s not recognized as valid PDB, XYZ, INT, or ARC file.",
              line));
        }
  ...
  logger.severe(String.
      format(" Exception in generating file list: %s \n %s", ex.toString(), ex.printStackTrace()));
}
  ...
  if (resultDirectory.exists()) {
    if (resultDirectory.isFile()) {
      logger.severe(String.format(
          " FFX shutting down: Pathname %s provided for directory points to a file.",
          resultDirectory.toString()));
    }
  } else {
    boolean success = resultDirectory.mkdirs();
    if (!success) {
      logger.severe(String.format(" FFX shutting down: could not create directory %s",
          resultDirectory.toString()));
    }
  }
...
if (resultsFile.exists()) {
  ...
  if (resultsFile.exists()) {
    logger.warning(String.
        format(" Versioning failed: appending xray.groovy.test.rescore file to end of file %s",
            resultsFile.getName()));
    versioned = false;
  }
}
...
Rescore rescorer = new Rescore(utils);
