...
class PlaceIon extends Script {
  ...
  PlaceIon run() {
        ...
        for (double ci = 0; ci <= 1.0; ci += spacing) {
          ...
          if (e < eMin) {
            ...
            logger.info(String.format(
                " New minimum %16.8f at (%12.6f %12.6f %12.6f) in %6.3f (sec)",
                eMin, realCoords[0], realCoords[1], realCoords[2], time * 1.0e-9))
          } else {
            logger.info(String.format(
                " Energy      %16.8f at (%12.6f %12.6f %12.6f) in %6.3f (sec)",
    ...
    if (eMin < Double.MAX_VALUE) {
      logger.info(String.
          format(" Best minimum   %16.8f found at (%16.8f %16.8f %16.8f)", eMin, minCoords[0],
              minCoords[1], minCoords[2]))
    }
    ...
    if (eMin < eStart) {
      ...
      diffractiondata.
          writeModel(FilenameUtils.removeExtension(modelfilename) + options.suffix + ".pdb")
      diffractiondata.
          writeData(FilenameUtils.removeExtension(modelfilename) + options.suffix + ".mtz")
    } else {
