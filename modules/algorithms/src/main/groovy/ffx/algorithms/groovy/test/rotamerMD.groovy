...
cli.l(longOpt: 'library', args: 1, argName: '1',
    'Available rotamer libraries are Ponder and Richards (1) or Richardson (2).');
cli.a(longOpt: 'algorithm', args: 1, argName: '1',
    'Algorithm for rotamer optimization; choices are independent residues (1), all with rotamer elimination (2), all brute force (3), sliding window (4), or box optimization (5).');
cli.iT(longOpt: 'iterations', args: 1, argName: '1', 'Number of total moves.');
cli.tH(longOpt: 'thermostat', args: 1, argName: 'Berendsen',
    'Thermostat: [Adiabatic / Berendsen / Bussi]');
cli.iN(longOpt: 'integrate', args: 1, argName: 'Beeman',
    'Integrator: [Beeman / RESPA / Stochastic]');
cli.w(longOpt: 'window', args: 1, argName: '7',
    'Size of the sliding window with respect to adjacent residues');
cli.i(longOpt: 'increment', args: 1, argName: '3',
    'Distance sliding window shifts with respect to adjacent residues');
cli.r(longOpt: 'cutoff', args: 1, argName: '2.0', 'The sliding window cutoff radius (Angstroms).')
cli.d(longOpt: 'direction', args: 1, argName: 'Forward',
    'Direction of the sliding window or box optimization (boxes indexed by increasing Z,Y,X): [Forward / Backward]');
cli.z(longOpt: 'undo', args: 1, argName: 'false',
    'Window optimizations that do not lower the energy are discarded.');
cli.s(longOpt: 'start', args: 1, argName: '-1',
    'Starting residue to perform the rotamer search on (-1 exits). For box optimization, first box to optimize.');
cli.f(longOpt: 'finish', args: 1, argName: '-1',
    'Final residue to perform the rotamer search on (-1 exits). For box optimization, last box to optimize.');
cli.c(longOpt: 'chain', args: 1, argName: 'A', 'Chain the residue selection belongs to.');
cli.m(longOpt: 'minimize', args: 1, argName: '0.01',
    'Minimize the final structure to the given RMS gradient (Kcal/mole/A).');
cli.t(longOpt: 'threeBody', args: 1, argName: 'true',
    'Include 3-Body interactions in elimination criteria.');
cli.g(longOpt: 'Goldstein', args: 1, argName: 'true',
    'True to use Goldstein Criteria, False to use DEE');
cli.p(longOpt: 'pruning', args: 1, argName: '2',
    'Prune no clashes (0), only single clashes (1), or all clashes (2).');
cli.e(longOpt: 'ensemble', args: 1, argName: '1',
    'Produce an ensemble of this many of the most favorable structures.');
cli.b(longOpt: 'buffer', args: 1, argName: '0.0/5.0',
    'Sets a starting energy buffer value for use with ensemble search.');
cli.x(longOpt: 'all', args: 1, argName: '1',
    'Optimize all residues in the system beginning from the passed residue number (overrides other options); for box optimization, optimizes all boxes from the passed index.');
cli.nt(longOpt: 'nucleicCorrectionThreshold', args: 1, argName: '0',
    'Nucleic acid Rotamers adjusted by more than a threshold distance (A) are discarded (0 disables this function).');
cli.mn(longOpt: 'minimumNumberAcceptedNARotamers', args: 1, argName: '10',
    'Minimum number of NA rotamers to be accepted if a threshold distance is enabled.');
cli.pf(longOpt: 'nucleicPruningFactor', args: 1, argName: '1.0',
    'Multiplier of pruning constraints for nucleic acids.');
cli.v(longOpt: 'verboseEnergies', args: 1, argName: 'true',
    'Calculates and prints beginning and default-conformation energies not necessary for the DEE algorithm');
cli.o(longOpt: 'includeOriginal', args: 1, argName: 'true',
    'Include starting coordinates as their own rotamer.');
cli.td(longOpt: 'threeBodyCutoffDist', args: 1, argName: '9.0',
    'Angstrom distance beyond which three-body interactions will be truncated (-1 for no cutoff).');
cli.dO(longOpt: 'decomposeOriginal', args: 1, argName: 'false',
    'Only print energy decomposition of original-coordinates rotamers; overrides other flags!');
cli.eR(longOpt: 'energyRestart', args: 1, argName: 'filename',
    'Load energy restart file from a previous run. Ensure that all parameters are the same!');
cli.nB(longOpt: 'numXYZBoxes', args: 1, argName: '3,3,3',
    'Specify number of boxes along X, Y, and Z');
cli.bB(longOpt: 'boxBorderSize', args: 1, argName: '3.0',
    'Extent of overlap between optimization boxes (Angstroms).');
cli.bL(longOpt: 'approxBoxLength', args: 1, argName: '0.0',
    'Approximate side lengths of boxes to be constructed (over-rides numXYZBoxes). Box sizes rounded up to make whole number of boxes along each axis. 0 disables this function.');
...
cli.bC(longOpt: 'boxInclusionCriterion', args: 1, argName: '1',
...
cli.fR(longOpt: 'forceResidues', args: 1, argName: '-1,-1',
    'Force residues in this range to be considered for sliding window radii, regardless of whether they lack rotamers.');
cli.sT(longOpt: 'superpositionThreshold', args: 1, argName: '0.1',
    'Sets the maximum atom-atom distance (Angstroms) which will cause a 2-Body or 3-Body energy to be defaulted to 1.0E100 kcal/mol.');
cli.sC(longOpt: 'singletonClashThreshold', args: 1, argName: '20.0',
    'Sets the threshold for singleton pruning.');
cli.pC(longOpt: 'pairClashThreshold', args: 1, argName: '50.0',
    'Sets the threshold for 2-body pruning');
cli.mB(longOpt: 'thermostat', args: 1, argName: 'Berendsen',
    'Thermostat: [Adiabatic / Berendsen / Bussi]');
cli.mD(longOpt: 'dt', args: 1, argName: '1.0', 'Time discretization (fsec).');
cli.mI(longOpt: 'integrate', args: 1, argName: 'Beeman',
    'Integrator: [Beeman / RESPA / Stochastic]');
...
RotamerLibrary rLib = new RotamerLibrary(RotamerLibrary.ProteinLibrary.intToProteinLibrary(library),
    true);
...
if (options.eR) {
  if (!parallelEnergies || algorithm == 4 || algorithm == 5) {
    logger.severe(
        " FFX shutting down: energy restart only implemented for parallelized global optimizations.");
  }
...
if (options.fR && algorithm == 4) {
  ...
  try {
    ...
    logger.severe(String.
        format(" FFX shutting down: input to -fR could not be parsed as a pair of integers: %s",
            input));
  }
  ...
  if (forceResiduesEnd < 1) {
    logger.severe(String.
        format(" FFX shutting down: end range for -fR must be at least 1; input range %d to %d",
            forceResiduesStart, forceResiduesEnd));
  }
}
...
Potential potential;
RotamerOptimization rotamerOptimization = new RotamerOptimization(active,
    active.getPotentialEnergy(), sh);
...
rotamerOptimization.setNucleicPruningFactor(pruningFactor);
...
for (int i = 0; i < numberOfIterations; i++) {
  ...
  MolecularDynamics molecularDynamics = new MolecularDynamics(active, active.getPotentialEnergy(),
