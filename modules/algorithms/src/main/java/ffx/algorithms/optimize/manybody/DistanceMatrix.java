...
public class DistanceMatrix {
     ...
     * residues.
     */
    private RotamerOptimization.DistanceMethod distanceMethod;
    /**
     ...
     */
    private boolean lazyMatrix;
    /**
     * Flag to indicate use of forced residues during the sliding window
     * algorithm.
     */
    private boolean useForcedResidues;

    public DistanceMatrix(RotamerOptimization rO, MolecularAssembly molecularAssembly, AlgorithmListener algorithmListener,
                          Residue[] allResiduesArray, ArrayList<Residue> allResiduesList, RotamerLibrary library,
                          RotamerOptimization.DistanceMethod distanceMethod, double distance,
                          double twoBodyCutoffDist, double threeBodyCutoffDist, boolean lazyMatrix, boolean useForcedResidues) {
        this.rO = rO;
    ...
    private void distanceMatrix() {
        ...
        for (int i = 0; i < (numResidues - 1); i++) {
            ...
            try {
                ...
                if (useForcedResidues) {
                    ...
                    rO.logIfMaster(format(" Non-forced Residue i %s has null rotamers.",
                ...
                for (int j = (i + 1); j < numResidues; j++) {
                    ...
                    try {
                        ...
                        if (useForcedResidues) {
                            ...
                            rO.logIfMaster(format(" Residue j %s has null rotamers.", residuej.toFormattedString(false, true)));
                        }
                        continue;
                    }
        ...
        if (!lazyMatrix) {
            ...
            try {
                distanceRegion.init(this, rO, molecularAssembly, allResiduesArray, library,
                        algorithmListener, distanceMatrix);
