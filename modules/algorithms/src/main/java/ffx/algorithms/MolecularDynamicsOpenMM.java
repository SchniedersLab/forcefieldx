...
public class MolecularDynamicsOpenMM extends MolecularDynamics {
    ...
    private long time;
    /**
     * Time elapsed for each molecular dynamics trajectory along with output of the calculated energies.
     */
    private long mdTime = 0;
    /**
     * Time elapsed to run the init method (will probably be removed once testing is complete).
     */
    private long initTime = 0;
    /**
     * Time elapsed for first update of OpenMM for a molecular dynamics move (will probably be removed once testing is complete).
     */
    private long firstUpdateTime = 0;
    /**
     * Time elapsed to take steps to generate the molecular dynamics trajectory (will probably be removed once testing is complete).
     */
    private long takeStepsTime = 0;
    /**
     * Time elapsed for the second update of OpenMM for a molecular dynamics move (will probably be removed once testing is complete).
     */
    private long secondUpdateTime = 0;
    /**
     * Time elapsed, in the dynamic method, from the beginning of the for loop to the end of the method (will probably be removed once testing is complete).
     */
    ...
    /**
     * Boolean used to signify molecular dynamics in the NVE (micro canonical) ensemble.
     */
    private boolean NVE = false;
    /**
     * Boolean used to signify molecular dynamics in the NPT (isothermal-isobaric) ensemble.
     */
    private boolean NPT = false;
    /**
     * Boolean used to suppress logging for creation of integrator, typically used to limit the amount of output to the screen.
     */
    private boolean quiet = true;
    /**
     * Double that holds the target pressure for the barostat under NPT dynamics.
     */
    ...
    /**
     * Integer used to count the number of times the context has been set up, mainly used to add a thermostat to the default integrator (Velocity Verlet).
     */
    ...
    /**
     * Array that holds the velocities generated by OpenMM under specific temperature.
     */
    private PointerByReference velArray = null;
    /**
     * Random number generator used to psuedo seed the OpenMM velocity generator method.
     */
    ...
    public final void updateContext() {
        ...
        if (currentTemperature != targetTemperature || currentTimeStp != dt || !currentIntegrator.equalsIgnoreCase(integratorString) || (currentIntegrator.equalsIgnoreCase("VERLET") && contextCounter != 0)) {
            ...
            fetchIntTime = -System.nanoTime();
            integrator = forceFieldEnergyOpenMM.getIntegrator();
            fetchIntTime += System.nanoTime();
            logger.info(String.format(" Retrieved integrator in %6.3f", fetchIntTime * NS2SEC));
            fetchContextTime = -System.nanoTime();
            context = forceFieldEnergyOpenMM.getContext();
            fetchContextTime += System.nanoTime();
            logger.info(String.format(" Retrieved context in %6.3f", fetchContextTime * NS2SEC));
        } else {
            long fetchIntTime = 0;
            long fetchContextTime = 0;
            fetchIntTime = -System.nanoTime();
