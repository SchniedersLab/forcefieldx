...
public class PhDiscount implements MonteCarloListener {
  ...
  /* Debug */
  private static final String[] keyPrefixes =
      new String[] {"phmd", "esv", "md", "ffe", "sys", "db", "sdl"};
   ...
   * titration, use Protonate. For traditional continuous titration, run mdesv for populations.
   *
  ...
  public PhDiscount(
    ...
    this.originalFilename =
        FilenameUtils.removeExtension(molecularAssembly.getFile().getAbsolutePath()) + "_dyn.pdb";
    ...
    logger.info(
        format(" Running DISCOuNT-pH dynamics @ system pH %.2f\n", esvSystem.getConstantPh()));
   ...
   */
  public void dynamic(
      int physicalSteps, double pH, double temperature, int attemptFrequency, int stepsPerMove) {
    ...
    logger.info(
        format(
            " (Round %d) Launching fixed-protonation dynamics for %d steps.",
    ...
    while (stepsTaken < physicalSteps) {
      ...
      logger.info(
          format(
              " (Attempt %d) Launching fractional-protonation dynamics for %d steps.",
    ...
    logger.info(
        format(
            " (Summary) DISCOUNT pHMD completed %d physical steps and %d moves, of which %d were accepted.",
   ...
   * {@inheritDoc}
   *
   * <p>Update the position of background atoms at each step, lest bonded term energies become
   * inconsistent.
   */
  @Override
  ...
  private boolean tryContinuousTitrationMove(int titrationDuration, double targetTemperature) {
    ...
    switch (titrationConfig.seedDistribution) {
      ...
      case DIRAC_CURRENT:
        ...
        for (int i = 0; i < numESVs; i++) {
          ...
          double current =
              (active.getAminoAcid3() == Titration.lookup(active).protForm) ? 1.0 : 0.0;
    ...
    // TODO: Need to ensure that we fully protonate the protein before entering
    // continuous-protonation space.
    ...
    logger.info(
        format(
            " %-40s %-s",
            "Trying continuous titration move.", format("Uo,Uo': %16.8f, %16.8f", Uo, Uo_prime)));
    ...
    logger.info(
        format(
            " %-40s %-30s",
            "Move finished; detaching esvSystem.", format("Un',Un: %16.8f, %16.8f", Un_prime, Un)));
    ...
    logger.info(
        format("   Un,Un',Uo',Uo:   %10.5f %10.5f %10.5f %10.5f", Un, Un_prime, Uo_prime, Uo));
    ...
    if (rand <= crit) {
      logger.info(
          format(
              " %-40s %-s",
              "Monte-Carlo accepted.", format("Wallclock: %8.3f sec", took * NS2SEC)));
      ...
      logger.info(
          format(
              " %-40s %-s",
              "Monte-Carlo denied; reverting state.",
