...
public class ThermodynamicsTest extends PJDependentTest {
  ...
  /**
   * Set of atoms for which tabulated gradients (dU/dL, d2U/dXdL) should be evaluated. Not used
   * alongside testing free energies, and values are meaningless (and often null) in that case.
   */
    ...
    if (doTest) {
      ...
      for (int i = 0; i < nFiles; i++) {
        ...
        for (String ext : copiedExtensions) {
          srcFile =
              new File(
                  String.format("%s.%s", FilenameUtils.removeExtension(srcFile.getPath()), ext));
          if (srcFile.exists()) {
            logger.fine(" Copying extension " + ext);
            tempFile =
                new File(
                    String.format("%s.%s", FilenameUtils.removeExtension(tempFile.getPath()), ext));
            logger.fine(String.format(" Copied file %s to %s", srcFile, tempFile));
            FileUtils.copyFile(srcFile, tempFile);
          }
        }
      ...
      if (nOpts > 0) {
        assertTrue(
            String.format("Unmatched option key %s for test %s", options[nOpts - 1], info),
            options.length % 2 == 0);
      }
      if (nProps > 0) {
        assertTrue(
            String.format("Unmatched property key %s for test %s", properties[nProps - 1], info),
            properties.length % 2 == 0);
      }
      if (nFlags > 0) {
        assertTrue(
            String.format("Unmatched flag key %s for test %s", flags[nFlags - 1], info),
            flags.length % 2 == 0);
      }
      ...
      for (int i = 0; i < nOpts; i += 2) {
        ...
        assertTrue(
            String.format(" Option %s for test %s does not look like a Groovy option!", opti, info),
      ...
      for (int i = 0; i < nProps; i += 2) {
        ...
        assertTrue(
            String.format(" Property %s for test %s does not look like a property!", propi, info),
      ...
      for (int i = 0; i < nFlags; i += 2) {
        ...
        assertTrue(
            String.format(" Flag %s for test %s does not look like a flag!", flagi, info),
      ...
      if (mode == ThermoTestMode.FREE) {
        assertTrue(
            String.format(" Free energy tolerance for test %s was %10.4g <= 0!", info, feTol),
            feTol > 0);
      }
    ...
    return Arrays.asList(
        new Object[][] {
          ...
          },
          {
            ...
            new String[] {
              "ffx/algorithms/structures/water-dimer.xyz", "ffx/algorithms/structures/water-na.xyz"
            },
            ThermoTestMode.GRAD,
            true,
            0,
          ...
          },
          {
            ...
            null,
            new String[] {
              "-d", "1.0", "-l", "0.5", "--ac", "1-3", "--ac2", "1", "-n", "20000", "-Q", "5000",
    ...
    if (doTest) {
      ...
      if (tempDir != null && tempDir.exists()) {
        if (tempDir.isDirectory()) {
          ...
          logger.warning(
              String.format(" Expected %s (temporary directory) to be a directory!", tempDir));
        }
        tempDir.delete();
      }
      ...
      switch (mode) {
        ...
        default:
          throw new IllegalStateException(
              String.format(" Thermodynamics test mode %s not recognized!", mode));
      }
    } else {
  ...
  private void testFreeEnergy() {
    ...
    assertEquals(
        String.format(" Test %s: not within tolerance %12.5g", info, feTol),
        freeEnergy,
        delG,
        feTol);
  }
  ...
  private EnergyResult testGradientSet(
    ...
    assertTrue(
        String.format(" Potential %s is not a lambda interface!", potential),
  ...
  private void checkThGradArray(
    ...
    for (int i = 0; i < numGradAtoms; i++) {
      ...
      if (debugMode) {
        logger.info(
            String.format(" %s at atom %d is %s", description, i, Arrays.toString(actualSlice)));
      } else {
