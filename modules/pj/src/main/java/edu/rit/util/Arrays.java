...
// Unit:    Class edu.rit.util.Arrays
//
...
// Exported array allocation operations.
    ...
    public static <T, ST extends T> void allocate(T[] array,
        ...
        if (0 > lb || ub >= array.length) {
            throw new IndexOutOfBoundsException("Arrays.allocate(): array indexes = 0.." + (array.length - 1)
                    + ", range = " + range);
        }
...
// Exported matrix allocation operations.
    ...
    public static void allocate(boolean[][] matrix,
        ...
        if (0 > rowLb || rowUb >= matrix.length) {
            throw new IndexOutOfBoundsException("Arrays.allocate(): matrix row indexes = 0.."
    ...
    /**
     * Allocate the elements within the given row index range in the given float
     * matrix. Each matrix row in the given row index range is allocated with
     * the given number of columns.
     *
     * @param matrix Matrix.
    ...
    public static <T, ST extends T> void allocate(T[][] matrix,
        ...
        if (0 > colLb || colUb >= ncols) {
            throw new IndexOutOfBoundsException("Arrays.allocate(): matrix column indexes = 0.." + (ncols - 1)
...
// Exported array deallocation operations.
    /**
     * Deallocate all elements in the given object array. Each array element is
     * set to null.
     *
     * @param <T> Array element data type.
     * @param array Array.
    ...
    public static <T> void deallocate(T[] array,
        ...
        int stride = range.stride();
        if (0 > lb || ub >= array.length) {
            throw new IndexOutOfBoundsException("Arrays.deallocate(): array indexes = 0.." + (array.length - 1)
                    + ", range = " + range);
        }
        for (int i = lb; i <= ub; i += stride) {
            array[i] = null;
...
// Exported matrix deallocation operations.
    ...
    public static void deallocate(boolean[][] matrix,
        ...
        int rowStride = rowRange.stride();
        if (0 > rowLb || rowUb >= matrix.length) {
            throw new IndexOutOfBoundsException("Arrays.deallocate(): matrix row indexes = 0.."
                    + (matrix.length - 1) + ", row range = " + rowRange);
        }
        for (int i = rowLb; i <= rowUb; i += rowStride) {
            matrix[i] = null;
    ...
    public static <T> void deallocate(T[][] matrix,
        ...
        for (int i = rowLb; i <= rowUb; i += rowStride) {
            ...
            if (0 > colLb || colUb >= ncols) {
                throw new IndexOutOfBoundsException("Arrays.allocate(): matrix column indexes = 0.."
    ...
    public static void copy(boolean[] src,
        ...
        if (srcStride == 1 && dstStride == 1) {
            ...
            if (0 > srcLower || srcUpper >= src.length) {
                throw new IndexOutOfBoundsException("Arrays.copy(): src indexes = 0.." + (src.length - 1)
                        + ", srcRange = " + srcRange);
            }
            if (0 > dstLower || dstUpper >= dst.length) {
                throw new IndexOutOfBoundsException("Arrays.copy(): dst indexes = 0.." + (dst.length - 1)
                        + ", dstRange = " + dstRange);
            }
            if (src != dst || srcLower > dstLower) {
...
// Exported matrix copying operations.
    ...
    public static void copy(boolean[][] src,
        ...
        if (0 > srcRowLower || srcRowUpper >= src.length) {
            throw new IndexOutOfBoundsException("Arrays.copy(): src row indexes = 0.." + (src.length - 1)
                    + ", srcRowRange = " + srcRowRange);
        }
        if (0 > dstRowLower || dstRowUpper >= dst.length) {
            throw new IndexOutOfBoundsException("Arrays.copy(): dst row indexes = 0.." + (dst.length - 1)
