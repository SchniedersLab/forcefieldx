...
// Hidden constructors.
     ...
     */
    private JobScheduler(String configfile)
            throws IOException {
        ...
        myLog
                = new PrintStreamLogger(new PrintStream(new FileOutputStream(config.getLogFile(), true),
                                true));
        ...
        myChannelGroup
                = new ChannelGroup(new InetSocketAddress(mySchedulerHost, mySchedulerPort),
                        myLog);
...
// Hidden operations.
    ...
    private synchronized void createJob(Channel theChannel) {
        ...
        // Start lease timers.
        jobinfo.renewTimer.start(Constants.LEASE_RENEW_INTERVAL,
...
// More hidden operations.
     ...
     * @exception IOException Thrown if an I/O error occurred.
     */
    private synchronized void renewTimeout(Timer theTimer,
     ...
     * @exception IOException Thrown if an I/O error occurred.
     */
    private synchronized void expireTimeout(Timer theTimer,
            JobFrontendRef theJobFrontend)
            throws IOException {
        if (theTimer.isTriggered()) {
            JobInfo jobinfo = getJobInfo(theJobFrontend);
            doCancelJob(System.currentTimeMillis(),
                    jobinfo,
     ...
     * @exception IOException Thrown if an I/O error occurred.
     */
    private synchronized void jobTimeout(Timer theTimer,
            JobFrontendRef theJobFrontend)
            throws IOException {
        if (theTimer.isTriggered()) {
            JobInfo jobinfo = getJobInfo(theJobFrontend);
    ...
    private JobInfo getJobInfo(JobFrontendRef frontend) {
        ...
        if (jobinfo == null) {
            jobinfo = new JobInfo(/*jobnum   */myNextJobNumber++,
     ...
     * @exception IOException Thrown if an I/O error occurred.
     */
    private void doFinishJob(long now,
            JobInfo jobinfo)
            throws IOException {
     ...
     * @exception IOException Thrown if an I/O error occurred.
     */
    private void doCancelJob(long now,
            JobInfo jobinfo,
            String errmsg)
            throws IOException {
     ...
     * @exception IOException Thrown if an I/O error occurred.
     */
    private void doCancelJobTooFewResources(long now,
            JobInfo jobinfo)
            throws IOException {
        String errmsg;
        if (jobinfo.Nt == 0) {
     ...
     * @exception IOException Thrown if an I/O error occurred.
     */
    private void doCleanupJob(long now,
            JobInfo jobinfo)
            throws IOException {
        // Stop lease timers.
     ...
     * @exception IOException Thrown if an I/O error occurred.
     */
    private void assignResourcesToJobs(long now)
            throws IOException {
            ...
            do {
                ...
                if (backendinfo.state == BackendInfo.State.IDLE
                    ...
                    for (int i = 0; i < Nproc; ++i) {
                        ...
                        // Inform Job Frontend.
                        jobinfo.frontend.assignBackend(/*theJobScheduler*/this,
     ...
     */
    private boolean enoughResourcesForJob(int Nn,
            int Np,
            int Nt) {
     ...
     * @param now Current time.
     */
    private void printStatusHtmlStart(PrintWriter out,
            long now) {
        out.println("<HTML>");
        out.println("<HEAD>");
        out.print("<TITLE>");
     ...
     * @param now Current time.
     */
    private synchronized void printStatusHtmlBody(PrintWriter out,
            long now) {
        out.println("<P>");
        out.println("<TABLE BORDER=0 CELLPADDING=0 CELLSPACING=0>");
        out.println("<TR>");
     ...
     */
    private void printJobNumberURL(PrintWriter out,
            int jobNum) {
     ...
     * @param jobNum Job number.
     */
    private void printJobNumberLink(PrintWriter out,
            int jobNum) {
     ...
     */
    private void printDeltaTime(PrintWriter out,
            long now,
            long then) {
     ...
     */
    private void printBackendInfo(PrintWriter out,
            long now,
            BackendInfo backend,
            int i) {
     ...
     * @param i Even = white background, odd = gray background.
     */
    private void printJobInfo(PrintWriter out,
            long now,
            JobInfo job,
            int i) {
        boolean first;
     ...
     * @param out Print writer.
     */
    private void printDebugHtmlStart(PrintWriter out,
            long now) {
        out.println("<HTML>");
        out.println("<HEAD>");
        out.print("<TITLE>");
     ...
     * @param jobNum Job number.
     */
    private void printJobDetailHtmlStart(PrintWriter out,
            long now,
            int jobNum) {
        out.println("<HTML>");
        out.println("<HEAD>");
     ...
     * @param jobNum Job number.
     */
    private synchronized void printJobDetailHtmlBody(PrintWriter out,
            long now,
            int jobNum) {
     ...
     */
    private void printJobDetailProcessInfo(PrintWriter out,
            JobInfo jobInfo,
            int rank) {
