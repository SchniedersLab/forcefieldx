...
// Package: edu.rit.pj
// Unit:    Class edu.rit.pj.LongStrideForLoop
//
 ...
 * loop's <code>finish()</code> method. Then the thread waits at a barrier. When all
 * the threads have reached the barrier, the <code>execute()</code> method returns.
 * <P>
 ...
 * argument. The parallel for loop's <code>run()</code> method must be coded this
 * way:
 * <PRE>
 *     public void run (long first, long last, long stride)
 *         {
...
// Exported operations.
     ...
     * by <code>stride</code> after each iteration.
     * <P>
    ...
    public final void ordered(ParallelSection theSection)
        ...
        if (theSection == null) {
            throw new IllegalStateException("LongStrideForLoop.ordered(): Parallel section is null");
        }
        if (myTeam == null) {
            throw new IllegalStateException("LongStrideForLoop.ordered(): No parallel team executing");
        }
