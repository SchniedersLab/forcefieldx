...
// Unit:    Class edu.rit.mp.buf.EmptyByteBuf
//
 ...
 * using the Message Protocol (MP). When a message is sent from an EmptyByteBuf,
 * the message item type is <code>byte</code> and the message length is 0. When a
 * message is received into an EmptyByteBuf, the message item type must be
 * <code>byte</code>, but all items in the message are discarded.
 *
