...
public class Keyword {
   ...
   *
   * <p>1.) Structure specific properties (for example pdbname.properties)
   *
   * <p>2.) Java system properties a.) -Dkey=value from the Java command line b.)
   * System.setProperty("key","value") within Java code.
   *
   * <p>3.) User specific properties (~/.ffx/ffx.properties)
   *
   * <p>4.) System wide properties (file defined by environment variable FFX_PROPERTIES)
   *
   * <p>5.) Internal force field definition.
   *
    ...
    if (properties.containsKey("key")) {
      ...
      String propertyFilename =
          (new File(structureBasename + ".properties").exists())
              ...
              : (new File(structureBasename + ".prop").exists())
                  ? structureBasename + ".prop"
                  : (new File(structureBasename + ".key").exists())
                      ? structureBasename + ".key"
                      : null;
      if (propertyFilename != null) {
        ...
        if (structurePropFile.canRead()) {
          try {
            ...
            PropertiesConfiguration propertyConfiguration = builder.getConfiguration();
            propertyConfiguration.setHeader(
                "Structure properties from (" + propertyFilename + ").");
            properties.addConfiguration(propertyConfiguration);
