 ...
 *
 * <p>Copyright: Copyright (c) Michael J. Schnieders 2001-2020.
 *
...
/**
 * Test ExtendedSystem and its variables to verify that (1) Analytic derivatives match finite
 * difference approximation to appropriate epsilon. (2) End state energies are consistent with
...
public class ExtendedVariableTest extends FFXTest {
  ...
  private static final String[] dilysStatesCryst = {
    "lyd-lyd-cryst.pdb", "lyd-lys-cryst.pdb", "lys-lyd-cryst.pdb", "lys-lys-cryst.pdb"
  };
  ...
  /**
   * Arguments to the class constructor. EsvTest.END_STATES requires two ESVs and that 4x states +
   * energies be defined. EsvTest.DERIVATIVES can take any system; use derivativeLambdas to control
   * FD points. EsvTest.SMOOTHNESS is ad-hoc; TODO: use d2U_dL2 from FD to assess smoothness
   * instead.
   */
  @Parameters
  ...
  /**
   * Set the system properties of which we are certain. This happens once, prior to class
   * construction; the resulting set defines revertSystemProperties().
   */
  @BeforeClass
  public static void setUpClass() {
    // Save passed-in configuration so we can restore it at teardown (and not affect downstream
    // tests).
    ...
    // ESV Settings
    // Include discr+pH biases; include bonded; hook up BG bonded to FG node; exclude BG atoms from
    // potential.
  ...
  private static String err(double analytic, double numeric, double threshold) {
    ...
    return (absError <= threshold && (percentError < 1.0 || numeric == 0.0))
        ? "nil"
  ...
  public MolecularAssembly setupWithExtended(
      String filename, boolean quietly, ExtendedSystemConfig esvConfig) {
    ...
    ExtendedSystem esvSystem =
        (esvConfig != null) ? new ExtendedSystem(mola, esvConfig) : new ExtendedSystem(mola);
      ...
      for (double lambda : lambdaValuesToTest) {
        ...
        if (oneSidedFiniteAtExtremes) {
          ...
          center =
              (lambda - step < 0.0)
                  ? lambda + step
                  : (lambda + step > 1.0) ? lambda - step : lambda;
          low = center - step;
          high = center + step;
        }
        ...
        final double permRealAna =
            esvSystem.getDerivativeComponent(PotentialComponent.PermanentRealSpace, i);
        final double permSelfAna =
            esvSystem.getDerivativeComponent(PotentialComponent.PermanentSelf, i);
        final double permRecipAna =
            esvSystem.getDerivativeComponent(PotentialComponent.PermanentReciprocal, i);
        final double indRealAna =
            esvSystem.getDerivativeComponent(PotentialComponent.InducedRealSpace, i);
        final double indSelfAna =
            esvSystem.getDerivativeComponent(PotentialComponent.InducedSelf, i);
        final double indRecipAna =
            esvSystem.getDerivativeComponent(PotentialComponent.InducedReciprocal, i);
        final double totalAna =
            esvSystem.getDerivativeComponent(PotentialComponent.ForceFieldEnergy, i);
        ...
        sb.append(
            format(
                " %-28s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s\n",
        ...
        sb.append(
            format(
                " %-28s %+12.6g %+12.6g %+12.6g %+12.6g %+12.6g %+12.6g %+12.6g %+12.6g %+12.6g %+12.6g\n",
        ...
        sb.append(
            format(
                ...
                "Error:",
                err(vdwAna, vdwNum),
                err(bondedAna, bondedNum),
                err(biasAna, biasNum),
                err(permRealAna, permRealNum),
                err(permSelfAna, permSelfNum),
                err(permRecipAna, permRecipNum),
                err(indRealAna, indRealNum),
                err(indSelfAna, indSelfNum),
                err(indRecipAna, indRecipNum),
                err(totalAna, totalNum)));
  ...
  /**
   * Verify that a lys-lys system with two ESVs can exactly reproduce the energy yielded by vanilla
   * energy() calls on mutated PDB files.
   */
  public void testEndStates() {
    ...
    final double[] permEsv = new double[4],
        permRealEsv = new double[4],
        permSelfEsv = new double[4],
        permRecipEsv = new double[4];
    final double[] directEsv = new double[4];
    final double[] mutualEsv = new double[4],
        indRealEsv = new double[4],
        indSelfEsv = new double[4],
        indRecipEsv = new double[4];
    final String[] esvStateNames = new String[4];

    final double[] decompPolarStateEsv =
        (decompPolarState == Polarization.MUTUAL) ? mutualEsv : directEsv;
    final double[] decompPolarCompEsv =
        (decompPolarComplement == Polarization.MUTUAL) ? mutualEsv : directEsv;
    ...
    final double[] decompPolarStateQi =
        (decompPolarState == Polarization.MUTUAL) ? mutualQi : directQi;
    final double[] decompPolarCompQi =
        (decompPolarComplement == Polarization.MUTUAL) ? mutualQi : directQi;
    final double[] decompPolarStateCart =
        (decompPolarState == Polarization.MUTUAL) ? mutualCart : directCart;
    final double[] decompPolarCompCart =
        (decompPolarComplement == Polarization.MUTUAL) ? mutualCart : directCart;
      ...
      for (int state = 0; state < numStates; state++) {
        ...
        sb.append(
            format(
                ...
                format(
                    "%-9s %-7.7s %10.5f",
                    name, esvStateNames[state], esvResult[component][state])));
        if (includeManualQiEndStates) {
          sb.append(
              format(
                  "    %-22s",
                  format("%-7.7s   %12.6f", stateFilenames[state], qiResult[component][state])));
        }
        sb.append(
            format(
                "    %-22s",
                format("%-7.7s   %12.6f", stateFilenames[state], cartResult[component][state])));
        final double error = Math.abs(cartResult[component][state] - esvResult[component][state]);
        final String errorStr =
            (error < errorThreshold) ? format("< %.1e", errorThreshold) : format("%+g", error);
  ...
  /**
   * Numerically ensure that the energy and lambda derivatives are smooth all along both ESV
   * coordinates in the dilysine system.
   */
  public void testSmoothness() {
    ...
    for (int idxA = 0; idxA <= 10; idxA++) {
      ...
      for (int idxB = 0; idxB <= 10; idxB++) {
        ...
        totalDerivsA[idxA][idxB] =
            esvSystem.getDerivativeComponent(PotentialComponent.ForceFieldEnergy, 0);
        totalDerivsB[idxA][idxB] =
            esvSystem.getDerivativeComponent(PotentialComponent.ForceFieldEnergy, 1);
        if (smoothnessDecomposition) {
          vdwEnergies[idxA][idxB] = ffe.getEnergyComponent(PotentialComponent.VanDerWaals);
          vdwDerivsA[idxA][idxB] =
              esvSystem.getDerivativeComponent(PotentialComponent.VanDerWaals, 0);
          vdwDerivsB[idxA][idxB] =
              esvSystem.getDerivativeComponent(PotentialComponent.VanDerWaals, 1);
          permanentEnergies[idxA][idxB] = ffe.getEnergyComponent(PotentialComponent.Permanent);
          permanentDerivsA[idxA][idxB] =
              esvSystem.getDerivativeComponent(PotentialComponent.Permanent, 0);
          permanentDerivsB[idxA][idxB] =
              esvSystem.getDerivativeComponent(PotentialComponent.Permanent, 1);
          inducedEnergies[idxA][idxB] = ffe.getEnergyComponent(PotentialComponent.Induced);
          inducedDerivsA[idxA][idxB] =
              esvSystem.getDerivativeComponent(PotentialComponent.Induced, 0);
          inducedDerivsB[idxA][idxB] =
              esvSystem.getDerivativeComponent(PotentialComponent.Induced, 1);
        }
      }
    }
    ...
    for (int idxB = 0; idxB < 10; idxB++) {
      ...
      if (absChange > maxChange && assertions) {
        org.junit.Assert.fail(
            format(
                "Failed max_change smoothness criterion: %g > %g. (change:%g->%g, range:{%g,%g})",
                absChange,
                maxChange,
                min,
                max,
                totalEnergies[testRow][idxB],
                totalEnergies[testRow][idxB + 1]));
      }
    }
    if (directionSignChanges > maxDirectionSignChanges && assertions) {
      org.junit.Assert.fail(
          format(
              "Failed direction_changes smoothness criterion: %d > %d.",
  ...
  private StringBuilder printAsTable(double[][] values, String title, StringBuilder sb) {
    ...
    sb.append(
        String.format(
            " %8s %8.1f %8.1f %8.1f %8.1f %8s %8s %8s %8.1f %8.1f %8.1f %8.1f",
