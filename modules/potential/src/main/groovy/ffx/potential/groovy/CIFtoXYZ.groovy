...
class CIFtoXYZ extends PotentialScript {
  ...
  CIFtoXYZ run() {
    ...
    for (CifCoreBlock block : cifFile.blocks) {
      ...
      if (zPrime > 1) {
        logger.info(format(
            " Detected more than one copy in asymmetric unit of CIF file (Z'=%d) -- attempting to separate.",
            zPrime))
      }
      ...
      if (itsFine) {
        ...
        if (zPrime == 1) {
          ...
          logger
              .fine(format(" Created %d bonds between CIF atoms (%d in xyz, so %d total molecules).",
                  cifBonds, xyzBonds, (int) cifBonds / xyzBonds))
          int numMolecules = (int) (cifBonds / xyzBonds)
          if (cifBonds % xyzBonds != 0) {
            logger.fine(format(" Created bonds (%d) do not match inputted molecule (%d).", cifBonds,
                xyzBonds))
          } else if (numMolecules != zPrime) {
            logger.fine(format(" Number of detected molecules (%d) does not equal expected (%d).",
                numMolecules, zPrime))
          }
          ...
          try {
            while (!atomPool.isEmpty()) {
              ...
              if (counter >= zPrime) {
                logger.info(format(
                    "ERROR: Molecule %d of %d detected, but %d atoms are ready and %d remain (%d atoms per molecule, %d atoms in CIF). ",
        ...
        for (int i = 0; i < zPrime; i++) {
          ...
          if (cifBonds % xyzBonds == 0) {
            ...
            if (p != null && p.length == nAUatoms) {
                ...
                if (hydrogen.isHydrogen()) {
                  ...
                  switch (anglesList.size()) {
                    ...
                    case 1:
                      ...
                      Atom atom3 = (atom1 == bonds2[0].get1_2(atom2)) ? bonds2[1].get1_2(atom2) :
                          bonds2[0].get1_2(atom2)
                      double diAng =
                    ...
                    default:
                      ...
                      if (angle0_2B == 0.0.toDouble()) {
                        ...
                        chiral = 0
                        List<Bond> bonds2 = atom2.getBonds()
                        atom2B = (atom1 == bonds2[0].get1_2(atom2)) ? bonds2[1].get1_2(atom2) :
                            bonds2[0].get1_2(atom2)
              ...
              if (p != null) {
                logger.info(
                    format(" Matched %d atoms out of %d in CIF (%d in XYZ)", p.length, nAUatoms,
                        xyzAtoms.size() - numHydrogens))
              }
            ...
            logger.warning(
                format(" CIF (%d) and XYZ ([%dH+%d=]%d) have a different number of bonds.", cifBonds,
        ...
        logger
            .warning(format(" CIF (%d) and XYZ (%d) files have different numbers of atoms.", nAtoms,
                nXYZAtoms))
        savePDB = true
      }
  ...
  private static String getAtomElement(String name) {
    return
    name.replaceAll("[()]", "").replaceAll("_", "").replaceAll("-", "").replaceAll(" +", "")
        .split("[0-9]")[0]
  }
