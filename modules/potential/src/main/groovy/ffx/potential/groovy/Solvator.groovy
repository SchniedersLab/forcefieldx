...
class Solvator extends PotentialScript {
  ...
  Solvator run() {
    ...
    for (int i = 0; i < 3; i++) {
      minSoluteXYZ[i] = Arrays.stream(soluteCoordinates).mapToDouble({double[] xyz -> return xyz[i]
      }).min().getAsDouble()
      maxSoluteXYZ[i] = Arrays.stream(soluteCoordinates).mapToDouble({double[] xyz -> return xyz[i]
      }).max().getAsDouble()
      soluteBoundingBox[i] = maxSoluteXYZ[i] - minSoluteXYZ[i]
    }

    double soluteMass = Arrays.stream(soluteAtoms).mapToDouble({
      it.getMass()
    }).sum()
    ...
    if (manualBox != null) {
      ...
      if (rectangular) {
        newBox = Arrays.stream(soluteBoundingBox).map({
          it + (2.0 * padding)
        }).toArray()
      } else {
    ...
    char solventChain = ' '
    Set<Character> soluteChains = Arrays.stream(soluteAtoms).map({
      it.getChainID()
    }).collect(Collectors.toSet())
    ...
    logger.info(" New solvent molecules will be placed in chain ${
      solventChain
    }")
    if (ionChain == ' '.charAt(0)) {
      logger.severe(" Could not find an unused character A-Z for the new solvent!")
    }
    logger.info(" New ions will be placed in chain ${
      ionChain
    }")
    ...
    if (ionFileName) {
      logger.info(" Ions will be placed into chain ${
        ionChain
      }")
      ...
      logger.info(" Charge before addition of ions is ${
        initialCharge
      }")
      ...
      for (IonAddition ia : byConc) {
        ...
        if (nIons > newMolecules.size()) {
          logger.severe(" Insufficient solvent molecules remain (${
            newMolecules.size()
          }) to add ${
            nIons
          } ions!")
      ...
      logger.info(" Charge before neutralization is ${
        initialCharge
      }")

      if (initialCharge > 0) {
        if (neutAnion == null) {
          logger.info(" No counter-anion specified; system will be cationic at charge ${
            initialCharge
          }")
        } else {
          logger.info(" Neutralizing system with ${
            neutAnion.toString()
          }")
        ...
        if (neutCation == null) {
          logger.info(" No counter-cation specified; system will be anionic at charge ${
            initialCharge
          }")
        } else {
          logger.info(" Neutralizing system with ${
