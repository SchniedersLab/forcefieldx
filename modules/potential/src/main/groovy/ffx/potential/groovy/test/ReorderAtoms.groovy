 ...
 */
@Command(description = " Reorder the atoms of an XYZ file based on atomic weight.", name = "ffxc test.ReorderAtoms")
class ReorderAtoms extends PotentialScript {

    /**
     * --sw or --swap Switch positions of all equivalent atoms (untested for >2 atoms).
     */
    @CommandLine.Option(names = ['--sw', '--swap'], paramLabel = "false", defaultValue = "false",
     ...
     */
    @CommandLine.Option(names = ['-o', '--overwrite'], paramLabel = "false", defaultValue = "false",
            ...
            for (int i = 0; i < numModels; i++) {
                ...
                for (int j = 0; j < alSize; j++) {
                    for (int k = j; k < alSize; k++) {
                        if (atomList0[j].getIndex() != atomList0[k].getIndex()) {
                            ...
                            if (value == 0) {
                                logger.info(" This " + atomList0[j].toString() + " and " + atomList0[k].toString() + " are \"equivalent\".")
                                if (swap) {
                ...
                for (int j = 0; j < nAtoms; j++) {
                    ...
                    if (logger.isLoggable(Level.FINE)) {
                        logger.fine(format(" Original index (%3d): %3d New index: %3d", j, originalOrder[j], atomList0[j].getIndex()))
                    }
                }
                // Current molecule
                ...
                XYZFilter filter = new XYZFilter(saveFile, newAssembly, assembly.getForceField(), assembly.getProperties())
                if (numModels > 1) {
