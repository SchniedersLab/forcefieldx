...
public abstract class MSGroup extends MSNode {
  ...
  /**
   * Constructs the Geometry of this MultiScaleGroup and stores the terms as children in the Term
   * node.
   */
  // Atoms Node
  ...
  /**
   * Abstract method that should specify how to add various MSNodes subclasses (such as Atoms,
   * Residues and Polymers) to the MSGroup
   *
  ...
  public void assignBondedTerms(ForceField forceField) {
    ...
    newOutOfPlaneBendNode.setName(
        "Out-of-Plane Bends (" + newOutOfPlaneBendNode.getChildCount() + ")");
    ...
    newStretchTorsionNode.setName(
        "Stretch-Torsions (" + newStretchTorsionNode.getChildCount() + ")");
    ...
    for (Bond bond : bonds) {
      PiOrbitalTorsion piOrbitalTorsion =
          PiOrbitalTorsion.piOrbitalTorsionFactory(bond, forceField);
    ...
    for (Atom atom : atoms) {
      if (atom.isTrigonal()) {
        List<ImproperTorsion> improperTorsions =
            ImproperTorsion.improperTorsionFactory(atom, forceField);
    ...
    newImproperTorsionNode.setName(
        "Improper Torsions (" + newImproperTorsionNode.getChildCount() + ")");
          ...
          for (Bond lastBond : atom3.getBonds()) {
            ...
            if (atom4 != atom0 && atom4 != atom1 && atom4 != atom2) {
              TorsionTorsion torsionTorsion =
                  TorsionTorsion.torsionTorsionFactory(firstBond, angle, lastBond, forceField);
    ...
    int numberOfValenceTerms =
        newBondNode.getChildCount()
            + newAngleNode.getChildCount()
            + newStretchBendNode.getChildCount()
            + newUreyBradleyNode.getChildCount()
            + newOutOfPlaneBendNode.getChildCount()
            + newTorsionNode.getChildCount()
            + newStretchTorsionNode.getChildCount()
            + newAngleTorsionNode.getChildCount()
            + newPiOrbitalTorsionNode.getChildCount()
            + newTorsionTorsionNode.getChildCount()
   ...
   * @param forceField the ForceField parameters to use when creating the joint.
   ...
   * @param forceField the ForceField parameters to use when finalizing the MSGroup.
   */
  ...
  public Atom getAtomByName(String n, boolean caseInsensitive) {
    ...
    for (MSNode msNode : list) {
      if (msNode.getName().equals(n) || (caseInsensitive && msNode.getName().equalsIgnoreCase(n))) {
        return (Atom) msNode;
      }
    }
    return null;
  ...
  /**
   * This method finds the Geometrical center of this MultiScaleGroup, or the atomicWeight-weighted
   * center if w is set to true, and returns it as a double[3].
   *
   * @param w a boolean.
   * @return an array of double.
   */
   ...
   * {@inheritDoc}
   *
   * <p>Prints the MultiScaleGroup's Atoms and Bonds.
   */
  @Override
  public void print() {
    super.print();
  ...
  /**
   * Getter for the field <code>joints</code>.
   *
   * @return a {@link java.util.List} object.
   */
