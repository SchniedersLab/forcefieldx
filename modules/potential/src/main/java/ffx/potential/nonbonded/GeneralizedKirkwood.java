...
public class GeneralizedKirkwood implements LambdaInterface {
   ...
   */
  private static final double DEFAULT_SNECK = 0.79;
  /**
   * Default value of beta0 for tanh scaling
   */
  private static final double DEFAULT_BETA0 = 0.39;
  /**
   * Default value of beta1 for tanh scaling
   */
  private static final double DEFAULT_BETA1 = 0.029;
  /**
  ...
  public void setGaussVolRadiiOffset(double offset) {
    ...
    if (gaussVol != null) {
      ...
      try {
        ...
        logger.severe(" Exception creating GaussVol: " + e);
      }
    }
  }
  ...
  public void applySoluteRadii() {
    // Set base radii and default HCT overlap scale factor.
    for (int i = 0; i < nAtoms; i++) {
      SoluteType soluteType = atoms[i].getSoluteType();
      if (soluteType == null) {
        logger.info(format(" No solute type for: %s", atoms[i]));
      }
      ...
      if (elementHCTScale) {
        ...
        if (elementHCTScaleFactors.get(elementName) != null && notIon) {
          ...
          logger.info("Element-specific HCT scale factor not found for atom " + i + " of element "
              + atomName);
          overlapScale[i] = gkOverlapScale;
        }
      }
    ...
    // Set up HCT overlap scale factors, descreen radii, and any requested radii overrides.
    for (int i = 0; i < nAtoms; i++) {
    ...
    // contribution to the mixed neck value should match that of their heavy atom.
    for (int i = 0; i < nAtoms; i++) {
      if (atoms[i].isHydrogen()) {
        int heavyAtomIndex = atoms[i].get12List().get(0).getXyzIndex() - 1;
