...
/**
 * <p>Initial implementation of an OldOctree decomposition.</p>
 *
 * @author Stephen D. LuCore
 * @since 1.0
 */
public class OldOctree {

    private static final Logger logger = Logger.getLogger(OldOctree.class.getName());
    ...
    public void debugPrintStats(boolean writePartitionFile, File partFile) {
        List<OldOctree> nodes = new ArrayList<>();
        List<OldOctree> leaves = new ArrayList<>();
        ...
        logger.info(" OldOctree Leaf Stats:");
    ...
    private void debugFindNodes(List<OldOctree> nodes) {
        nodes.add(this);
        if (children != null) {
            for (OldOctree child : children) {
    ...
    private void split() {
        ...
        double y = corner.y;
        double z = corner.z;
        children = new OldOctree[8];
        children[0] = new OldOctree(depth + 1,
                new Point3d(x, y, z), sel);
        children[1] = new OldOctree(depth + 1,
                new Point3d(x + sel, y, z), sel);
        children[2] = new OldOctree(depth + 1,
                new Point3d(x, y + sel, z), sel);
        children[3] = new OldOctree(depth + 1,
                new Point3d(x, y, z + sel), sel);
        children[4] = new OldOctree(depth + 1,
                new Point3d(x + sel, y + sel, z), sel);
        children[5] = new OldOctree(depth + 1,
                new Point3d(x + sel, y, z + sel), sel);
        children[6] = new OldOctree(depth + 1,
                new Point3d(x, y + sel, z + sel), sel);
        children[7] = new OldOctree(depth + 1,
    ...
    private void addAtom(Atom atom) {
        ...
        if (contents.size() > maxAtomsPerVolume && depth < maxTreeDepth) {
            if (children != null) {
                if (!warnedAboutSplit) {
                    logger.warning("OldOctree couldn't split due to straddlers; maxAtomsPerVolume may be violated.");
     ...
     *
     * @return an array of {@link OldOctree} objects.
     */
    public OldOctree[] getChildren() {
        return children;
    }
