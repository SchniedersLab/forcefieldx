...
public final class ExtUtils {
  ...
  /**
   * Returns the result of calling toString() on each element of the array. Returned type is
   * Object[] (rather than String[]) to support use as varargs. Example: format("%s",
   * Arrays.toString( array ); // GOOD format("%s %s %s %s", Arrays.toString( array ); // FAIL
   *
   * <p>format("%s", arrayToStrings( array ); // FAIL format("%s %s %s %s", arrayToStrings( array );
   * // GOOD
   *
  ...
  /**
   * Element-wise sum over a list of 1D double arrays. This implementation benchmarks faster than
   * the equivalent Java8 stream() API.
   */
  private static double[] eleSum1DArrays(List<double[]> terms, int numESVs) {
    ...
    for (int iTerm = 0; iTerm < terms.size(); iTerm++) {
      ...
      if (currentTerm.length != numESVs) {
        logger.warning(
            format("iTerm %d length: %d, numESVs: %d", iTerm, terms.get(iTerm).length, numESVs));
