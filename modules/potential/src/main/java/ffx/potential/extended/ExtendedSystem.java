...
public class ExtendedSystem implements Potential {
  ...
  /**
   * Coefficients that define the per residue type Fmod polynomial quadratic * titrationLambda^2 +
  ...
  /**
   * Coefficients that define the tautomer component of the bivariate Histidine Fmod quadratic *
   * tautomerLambda^2 + linear * tautomerLambda
   */
  ...
  /**
   * ForceField Energy Instance. This instance is only used for Potential implementations for
   * grabbing the energy components.
   */
  ...
  /**
   * Array of booleans that is initialized to match the number of atoms in the assembly to note
   * whether an atom is specifically a heavy atom with changing polarizability (CYS SG, ASP OD1/OD2,
   * GLU OE1/OE2).
   */
  ...
  /**
   * Current state of the ESV system including theta coordinates, velocities, and accelerations.
   */
  ...
  public ExtendedSystem(MolecularAssembly mola, double pH, final File esvFile) {
    ...
    lockStates = properties.getBoolean("lock.esv.states",
        false); // Prevents setTitrationLambda/setTautomerLambda
    ...
    ASHquadratic = properties.getDouble("ASH.quadratic",
        TitrationUtils.Titration.ASHtoASP.quadratic);
    ...
    GLHquadratic = properties.getDouble("GLH.quadratic",
        TitrationUtils.Titration.GLHtoGLU.quadratic);
    ...
    LYSquadratic = properties.getDouble("LYS.quadratic",
        TitrationUtils.Titration.LYStoLYD.quadratic);
    ...
    CYSquadratic = properties.getDouble("CYS.quadratic",
        TitrationUtils.Titration.CYStoCYD.quadratic);
    ...
    HIDquadratic = properties.getDouble("HID.quadratic",
        TitrationUtils.Titration.HIStoHID.quadratic);
    HIDlinear = properties.getDouble("HID.linear", TitrationUtils.Titration.HIStoHID.linear);
    HIEquadratic = properties.getDouble("HIE.quadratic",
        TitrationUtils.Titration.HIStoHIE.quadratic);
    HIElinear = properties.getDouble("HIE.linear", TitrationUtils.Titration.HIStoHIE.linear);
    HIDtoHIEquadratic = properties.getDouble("HIDtoHIE.quadratic",
        TitrationUtils.Titration.HIDtoHIE.quadratic);
    HIDtoHIElinear = properties.getDouble("HIDtoHIE.linear",
        TitrationUtils.Titration.HIDtoHIE.linear);
    ...
    for (Residue residue : residueList) {
      if (isTitratable(residue)) {
        ...
        for (Atom atom : atomList) {
          ...
          isTitratingHydrogen[atomIndex] = TitrationUtils.isTitratingHydrogen(
              residue.getAminoAcid3(), atom);
          isTitratingHeavy[atomIndex] = TitrationUtils.isTitratingHeavy(residue.getAminoAcid3(),
              atom);
          ...
          if (isTitratingHeavy(atomIndex)) {
            double deprotPolar = titrationUtils.getPolarizability(atom, 0.0, 0.0,
                atom.getPolarizeType().polarizability);
            double protPolar = titrationUtils.getPolarizability(atom, 1.0, 1.0,
                atom.getPolarizeType().polarizability);
        ...
        if (isTautomer(residue)) {
          ...
          for (Atom atom : atomList) {
            ...
            if (isTitratingHydrogen[atomIndex]) {
              logger.info(
                  "Residue: " + residue + " Atom: " + atom + " atomType: " + atom.getAtomType().type
                      + " " + atom.getAtomType().atomClass);
            }
   ...
   * @param changeThetas whether or not to change the theta positions ~comes with information
   *     loss~
   */
  ...
  /**
   * Questions whether the current atom's polarizability is changing in response to lambda being
   * updated. Only affects carboxylic oxygen and sulfur.
   *
   * @param atomIndex
   * @return
   */
  ...
  /**
   * Sets the restartFile field of this extended system to the passed file. This does not read the
   * file, it only determines where the writeRestartFile() will write to.
   *
   * @param esvFile
   */
  ...
  /**
   * Writes out the current state of the extended system to the specified file without setting the
  ...
  public boolean writeESVInfoTo(File esvFile) {
    logger.info("Writing pH Dynamics out to: " + esvFile.getParentFile().getName() + File.separator
        + esvFile.getName());
    double[] thetaPosition = esvState.x();
    double[] thetaVelocity = esvState.v();
    double[] thetaAccel = esvState.a();
   ...
   * <p>
   * CAUTION: If the old data is not written out to file before this is called, the data will be
   * lost.
   *
  ...
  public void printProtonationRatios() {
    for (int i = 0; i < esvHistogram.length; i++) {
      ...
      logger.info(" " + extendedResidueList.get(i).toString() + " Deprotonation Fraction at pH "
          + constantSystemPh + ": " + (rowSums[0] / (i1 + buf)));
      if (buf == 0.0) {
  ...
  public void writeLambdaHistogram(boolean printHistograms) {
    ...
    if (printHistograms) {
      logger.info(
          esvFilter.getLambdaHistogram(titratingResidueList, esvHistogram, constantSystemPh));
    }
  }
