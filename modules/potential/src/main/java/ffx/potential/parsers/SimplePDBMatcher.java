...
public class SimplePDBMatcher {
    ...
    public void match() {
        ...
        for (int i = 0; i < matchFiles.length; i++) {
            ...
            try {
                ...
                logger.warning(format(" Matching failed for file %s", matchFile.getName()));
            }
        }
        for (int i = 0; i < matchFiles.length; i++) {
            logger.info(format(" Match file %s best source: %s at %11.7f A",
    ...
    private FileDoublePair loopOverSources(PDBFileReader reader, StructurePairAligner aligner, Structure matchStructure, String matchName) {
        ...
        for (File sourceFile : sourceFiles) {
            try {
                ...
                logger.warning(format(" Source file %s could not be read", sourceFile.toString()));
            }
        }
        logger.info(format(" Minimum RMSD for file %s: %11.7f to file %s", matchName, rmsd, bestMatch.getName()));
    ...
    private double checkRMSD(Structure matchStructure, Structure sourceStructure, StructurePairAligner aligner, String matchName, String sourceName) {
        ...
        try {
            ...
            logger.info(format(" Minimum RMSD for match %s source %s: %11.7f", matchName, sourceName, bestRMSD));
        } catch (StructureException ex) {
            logger.warning(format(" Structure exception during match %s source file %s", matchName, sourceName));
        }
        return bestRMSD;
    }
    ...
    private class FileDoublePair {
        ...
        FileDoublePair(File file, double rmsd) {
            this.file = file;
            this.rmsd = rmsd;
        }
    }
