...
/**
 * Class ProgressiveAlignmentOfCrystals holds the majority of the functionality necessary to quantify crystal similarity following
 * the PACCOM method.
 *
...
public class ProgressiveAlignmentOfCrystals{
     ...
     */
    private Comm world = null;
    /**
     * Number of processors available (typically called size).
     */
    private final Integer numProc;
    /**
     * Rank of this process.
     */
    private final int rank;
    /**
     * The distances array stores communicated parameters for each process (dimensions compatible with comparable calc).
     * (i.e. crystal comparisons).
     * Currently the array is of size [number of Processes][comparisons/processes].
     */
    ...
    /**
     * Matrix to store comparisons (human readable mxn matrix).
     */
    public final double[][] distMatrix;
    /**
     * Array of distance comparisons this process is responsible for.
     */
    private static double[] values;
    /**
     * Wrapper used to share the "values" parameter with the rest of the processes.
     */
    private static DoubleBuf myBuf;
    /**
     * Temporary output file to store comparison values if restart is needed.
     */
    ...
    private int numMaxUnique = 0;

    /**
     * Constructor for the ProgressiveAlignmentOfCrystals class.
     *
     * @param systemFilter  SystemFilter containing a set of crystal structures to compare.
     * @param systemFilter2 SystemFilter containing the other set of crystals to compare.
     */
    public ProgressiveAlignmentOfCrystals(SystemFilter systemFilter, SystemFilter systemFilter2) {
        this.systemFilter = systemFilter;
        this.systemFilter2 = systemFilter2;


        try {
            // Set up the communication variables for Parallel Java communication between nodes.
            world = Comm.world();
        ...
        baseSize = systemFilter.countNumModels();
        targetSize = systemFilter2.countNumModels();
        logger.info(format(" NumProc: %d", numProc));
        logger.info(format(" Base Size: %d", baseSize));
        logger.info(format(" Target Size: %d", targetSize));
        ...
        for(int i = 0; i<baseSize; i++){
            Arrays.fill(distMatrix[i], -1.0);
        }

        // Allocate array to hold distances computed by each process (some may be unused).
        int size = (int) Math.ceil((baseSize * targetSize) / numProc.doubleValue());
        logger.info(format(" Distances Size: %d", size));
        distances = new double[numProc][size];
        // Initialize array as -1.0 as -1.0 is not a viable RMSD.
        for(int i = 0; i<numProc; i++){
            Arrays.fill(distances[i], -1.0);
        }
        ...
        values = distances[rank];
        myBuf = buffers[rank];
        myFile = new File( rank + ".log");
    }
     ...
     * @param write             Write out distance matrix to a file (dist.txt).
     ...
     */
    public double[][] comparisons(int nAtoms, List<Integer> comparisonAtoms, int nAU, int inflatedAU, int numSearch,
                                  int numSearch2, boolean write, boolean force, boolean symmetric, boolean save) {
        //Ideally comparisons would be symmetric. The difference between two crystals should be fixed regardless of order.
        //  The prioritization of molecules depends on which system is first, unless symmetric == true. Then
        //  prioritizations are done by whichever crystal has the closest 2nd and 3rd replicates to the center.

        // Parallel Java Environment file records number of nodes being used for the calculation. If it is present,
        //   then the previous job did not finish, and this run should pick up where the last one left off.
        // First node is responsible for determining restart information...
        if(rank==0) {
            File env = new File("pjenv.log");
            int procCount = -1;
            if (env.exists()) {
                try {
                    // If exsists record number of processes from previous calculation.
                    BufferedReader br = new BufferedReader(new FileReader(env));
                    procCount = Integer.parseInt(br.readLine());
                    br.close();
                    if(!env.delete()){
                        logger.warning(" Could not delete PJ environment file.");
                    }
                } catch (Exception ex) {
                    logger.warning(" PJ environment file exists but was not found.");
                    logger.info(ex.toString());
                }
            }
            // Create new file to record current number of processes.
            try {
                if (env.createNewFile()) {
                    BufferedWriter bw = new BufferedWriter(new FileWriter(env));
                    bw.write(Integer.toString(numProc));
                    bw.close();
                } else {
                    logger.warning(" Could not create the PJ environment file.");
                }
            } catch (Exception ex) {
                logger.warning(" Could not create new PJ environment file.");
                logger.info(ex.toString());
            }

            // If previous job was found gather comparison outputs and add them to oldDistances.
            double[][] oldDistances;
            if (procCount > 0) {
                oldDistances = new double[procCount][(int) Math.ceil((baseSize * targetSize) / numProc.doubleValue())];
                for (int i = 0; i < procCount; i++) {
                    try {
                        BufferedReader br = new BufferedReader(new FileReader( i + ".log"));
                        String data;
                        while ((data = br.readLine()) != null) {
                            String[] tokens = data.trim().split(" +");
                            for (int j = 0; j < tokens.length; j++) {
                                oldDistances[i][j] = Double.parseDouble(tokens[j]);
                            }
                        }
                    } catch (Exception ex) {
                        logger.warning(format(" Processor temporary output file not found: .%d.log", i));
                        logger.info(ex.toString());
                    }
                }
            } else {
                // If procCount was not updated based on previous file, oldDistances equals current distances.
                oldDistances = distances;
            }
            // If continuing old calculation, add previous values to distMatrix. Otherwise should all be zeroes...
            updateMatrix(oldDistances, distMatrix);
            // Update distances with previous values.
            updateDistances(distMatrix, distances);
            env.deleteOnExit();
        }else{
            // All other nodes wait for first node to finish.
            try {
                wait(1000);
            }catch(Exception ex){
                logger.info(ex.toString());
        ...
        try{
            world.scatter(0, buffers, myBuf);
        }catch(Exception ex){
            logger.info(ex.toString());
        }

        for (int baseCount = 0; baseCount < baseSize; baseCount++) {
            logger.info(format(" Base Count: %d", baseCount));
            for (int targetCount = 0; targetCount < targetSize; targetCount++) {
                logger.info(format(" Target Count: %d", targetCount));
                if ((baseCount * targetSize + targetCount) % numProc == rank) {
                    time(false);
                    compare(nAtoms, comparisonAtoms, nAU, inflatedAU, numSearch, numSearch2, force,
                            symmetric, save, baseCount, targetCount);
                    double time = time(true);
                    if(time < minTime){
                        minTime = time;
                    }
                }
                // Check for termination request.
                systemFilter2.readNext(false, false);
            }
            systemFilter2.readNext(true, false);
            systemFilter.readNext(false, false);
        }

        logger.info(format(" Minimum calc time: %4.4f", minTime));

        systemFilter.closeReader();
        systemFilter2.closeReader();
        // Gather all parameters from the other processes.
        if(world!=null) {
            try {
                world.allGather(myBuf, buffers);
            } catch (IOException ex) {
                String message = " Progressive Alignment Algorithm allGather failed.";
                logger.log(Level.SEVERE, message, ex);
            }
        }
        updateMatrix(distances, distMatrix);
        if(write) {
            writeMatrix(distMatrix, baseSize, targetSize);
        }

        // All comparisons have finished for this node. Therefore delete stored values.
        if(!myFile.delete()){
            logger.warning(format(" Processor (%d) output file could not be deleted.", rank));
        }

        // Print out human readable matrix (fewer sig fig to better display on screen).
        if (baseSize > 1 || targetSize > 1) {
            StringBuilder sBPrint = new StringBuilder();
            for (double[] row : distMatrix) {
                for (double col : row) {
                    sBPrint.append(format("%8.4f  ", col));
                }
                sBPrint.append("\n");
            }
            logger.info("\n Distance Matrix:\n" + sBPrint);
     ...
     * @param baseCount Desired crystal from File 1.
     * @param targetCount Desired crystal from File 2.
     */
    private void compare(int nAtoms, List<Integer> comparisonAtoms, int nAU, int inflatedAU,
        ...
        int procIndex = Math.floorDiv(baseCount * targetSize + targetCount, numProc);
        // Initialized to -1 so if 0.0 or greater it was calculated previously.
        MolecularAssembly baseAssembly = systemFilter.getActiveMolecularSystem();
        if (baseSize > 1) {
            logger.info(format("\n Base %2d of %2d.", baseCount + 1, baseSize));
        }
        ...
        MolecularAssembly molecularAssembly = systemFilter2.getActiveMolecularSystem();
        if (targetSize > 1) {
            logger.info(format(" Target %2d of %2d", targetCount + 1, targetSize));
        }

        MolecularAssembly targetSphere = generateInflatedSphere(molecularAssembly, inflatedAU);
        logger.info(format(" Number AUs compared: %6d %6d %6d", 1, 3, nAU));
        int nTargetMols;
        ...
        prioritizeReplicates(compareAtomsSize, targetXYZ, mass, massSum, nTargetMols, molDist2, molDist22);
        ...
        for (int l = 0; l < numSearch; l++) {
            ...
            prioritizeReplicates(compareAtomsSize, baseXYZ, mass, massSum, nBaseMols, molDist1_2, center, molDist12_2);
            ...
            for (int m = 0; m < numSearch2; m++) {
                ...
                prioritizeReplicates(compareAtomsSize, targetXYZ, mass, massSum, nTargetMols, molDist2_2, center, molDist22_2);
                ...
                if (checkRMSD0 < 5.0 || force) {
                    ...
                    firstRotation(baseXYZ, targetXYZ, mass, molDist12_2[0].getIndex(), molDist22_2[0].getIndex());
...
//                    }
                    ...
                    if (logger.isLoggable(Level.FINER)) {
                        ...
                        for (int j = 0; j < numCheck; j++) {
                            ...
                            logger.finer(format(" Position: %d Base(%4.4f): %8.4f %8.4f %8.4f Target(%4.4f): %8.4f %8.4f %8.4f", j,
                                    molDist12_2[j].getDoubleValue(), baseCenters[j][0], baseCenters[j][1], baseCenters[j][2],
                                    molDist22_2[j].getDoubleValue(), targetCenters[j][0], targetCenters[j][1], targetCenters[j][2]));
                        }
                    }
                    ...
                    if (logger.isLoggable(Level.FINER)) {
                        ...
                        for (int i = 0; i < printSize; i++) {
                            logger.finer(format(" %2d %16.8f", matchMols[i].getIndex(), matchMols[i].getDoubleValue()));
                        }
                    ...
                    logger.info(format(" RMSD_%d (%2d of %2d): %4.4f %4.4f %4.4f", nAU, l * numSearch2 + m + 1,
                    ...
                    logger.fine(format(" Center molecule %2d has an RMSD of %16.8f and was skipped.", m, checkRMSD0));
                }
            }
        }

        logger.fine(format(" Processor Index: %d", procIndex));
        if (bestRMSD < Double.MAX_VALUE) {
            logger.info(format(" Best RMSD: %16.8f", bestRMSD));
            values[procIndex] = bestRMSD;
        } else {
            values[procIndex] = Double.NaN;
        ...
        if (logger.isLoggable(Level.INFO)) {
            ...
            if(numUnique > numMaxUnique){
                numMaxUnique = numUnique;
            }
            logger.info(format(" Number of Unique RMSD Distances: %d", numUnique));
            if(numUnique > 4){
                logger.warning(format(" PAC determined %2d unique values. Please increase number of inflated molecules."
                        , numUnique));
            }
    ...
    /**
     * Update human readable matrix (dimensions m x n) with parallel java compatible array (dimensions numProc x values).
     * @param distances Parallel java compatible array (numProc x numComparisons/numProc).
     * @param distMatrix Human readable array (m x n crystals for comparison).
     */
    private static void updateMatrix(double[][] distances, double[][] distMatrix){
        int baseSize = distMatrix.length;
        int targetSize = distMatrix[0].length;
        int numProc = distances.length;
        int numValues = distances[0].length;
        // Create n x n matrix for printing...
        for (int i = 0; i<numProc; i++) {
            for (int j = 0; j<numValues; j++) {
                int k = Math.floorDiv(i + j * numProc, targetSize);
                int l = (i + j * numProc) % targetSize;
                // When discrepancies between the sizes of distMatrix and distances, distances will be larger.
                //   Some values in distances may be zero (no calulation).
                if(k<baseSize && l<targetSize) {
                    distMatrix[k][l] = distances[i][j];
                }
            }
        }
    }
     ...
     * @param distances Parallel java compatible array (numProc x numComparisons/numProc).
     */
    private static void updateDistances(double[][] distMatrix, double[][] distances){
        int baseSize = distMatrix.length;
        int targetSize = distMatrix[0].length;
        int numProc = distances.length;
        // Create n x n matrix for printing...
        for (int i = 0; i<baseSize; i++) {
            for (int j = 0; j<targetSize; j++) {
                int k = (i * targetSize + j) % numProc;
                int l = Math.floorDiv(i * targetSize + j, numProc);
                distances[k][l] = distMatrix[i][j];
            }
        }
    }
     ...
     * @param distMatrix Array to write out to file.
     */
    private static void writeMatrix(double[][] distMatrix, int baseCount, int targetCount) {
        // Create formatted string builder to contain output.
        ...
        for (double[] row : distMatrix) {
            for (double col : row) {
                ...
                if(++targetValue > targetCount){
                    if(++baseValue > baseCount){
                        break;
                    }else{
                        targetValue = 0;
                    }
        ...
        try {
            File distTxt = new File("dist.txt");
            FileWriter fw = new FileWriter(distTxt);
     ...
     */
    private static void matchMolecules(DoubleIndexPair[] matchMols, double[] baseXYZ, double[] targetXYZ,
        ...
        if (logger.isLoggable(Level.FINE)) {
            ...
            for (int i = 0; i < desiredMols; i++) {
                logger.fine(format(" Base position:   %d: %8.4f %8.4f %8.4f", i, baseMols[i][0], baseMols[i][1], baseMols[i][2]));
                logger.fine(format(" Match Distance:  %d: %8.4f", i, matchMols[i].getDoubleValue()));
                logger.fine(format(" Target position: %d: %8.4f %8.4f %8.4f", i, targetCenterMols[i][0], targetCenterMols[i][1], targetCenterMols[i][2]));
            }
        }
    }

    /**
     * Calculate the center of mass for a given set of masses for the asymmetric unit and coordinates (xyz)
     ...
     */
    private static void centerOfMass(double[][] centersOfMass, double[] coords, double[] mass, DoubleIndexPair[] order){
    ...
    /**
     * Generate and expanded sphere of asymetric unit with the intention of observing a crystals distribution of
     * replicates rather to facilitate comparisons that go beyond lattice parameters.
     *
     ...
     */
    private static MolecularAssembly generateInflatedSphere(MolecularAssembly molecularAssembly, int inflatedAU) {
        ...
        // Add wiggle room for desired radius.
        double inflationFactor = asymmetricUnitVolume/inflatedAU;
...
//        double radius = cbrt(((3.0 / (4.0 * PI)) * inflatedAU * asymmetricUnitVolume));
        ...
        logger.info(format(" Number of replicates: %3d", nSymm));
        ...
        double[] cartCenter = new double[3];
            ...
            logger.fine(format(" Expanded Crystal Center: %16.8f %16.8f %16.8f", cartCenter[0], cartCenter[1], cartCenter[2]));
    ...
    /**
     * Orient coordinates so that the second index is on the x axis, and the third index is on the X-Y plane.
     * First index should be at the origin (0, 0, 0).
     *
     * @param coordsXYZ   An array of XYZ positions (e.g. [x0, y0, z0, x1, y1, z1, x2, y2, z2]
     * @param atomIndices Indices for three desired sets from the XYZ list (e.g. [0, 1, 2]). Index 0 should be at origin.
     */
    ...
    public static void standardOrientation(double[] coordsXYZ, int[] atomIndices) {
        ...
        logger.fine(format(" START: N1:\t%16.15f %16.15f %16.15f", atomCoords[0], atomCoords[1], atomCoords[2]));
        logger.fine(format(" START: N2:\t%16.15f %16.15f %16.15f", atomCoords[3], atomCoords[4], atomCoords[5]));
        logger.fine(format(" START: N3:\t%16.15f %16.15f %16.15f", atomCoords[6], atomCoords[7], atomCoords[8]));
        ...
        logger.finer(format(" DONE N1: %16.15f %16.15f %16.15f", atomCoords[0], atomCoords[1], atomCoords[2]));
    ...
    /**
     * Produce a translation vector necessary to move an object with the current center of mass (com) into the
     * provided crystal.
     *
    ...
    private static double[] moveIntoCrystal(Crystal cryst, double[] com) {
        ...
        logger.finest(format(" Center of Mass Prior: %16.8f %16.8f %16.8f", currentCoM[0], currentCoM[1], currentCoM[2]));
     ...
     * @param molDists         DoubleIndexPair of distances from center most asymmetric unit to asymmetric unit at index.
     */
     ...
     * @param molDists      Prioritization of molecules in expanded system on distance to center-most molecule.
    ...
    private static void prioritizeReplicates(int nAtoms, double[] coordsXYZ, double[] mass, double massSum, int nMols,
        ...
        if (logger.isLoggable(Level.FINER)) {
            ...
            for (int i = 0; i < numCheck; i++) {
                logger.finer(format("Rank %d Target: %d Index: %d Dist %4.4f (%4.4f %4.4f %4.4f)", i, index, molDists[i].getIndex(),
    ...
    /**
     * Add a value to a list of long if it's difference to all listed values is greater than the tolerance.
     ...
     */
    private static void saveAssemblyPDB(MolecularAssembly molecularAssembly, double[] coords, String description) {
        ...
        PDBFilter pdbfilter = new PDBFilter(saveLocationPDB, currentAssembly, forceField, currentAssembly.getProperties());
     ...
     *
     */
    public double time(boolean print) {
