...
public class ProgressiveAlignmentOfCrystals {
  ...
  private double compare(final File file1, final String name1, final List<Bond> bondList1,
    ...
    if (logger.isLoggable(Level.FINER)) {
      ...
      for (int i = 0; i < 10; i++) {
        stringBuilder.append(format(" %2d %2d\t%16.8f\n", i, baseAUDist[i].index(),
            sqrt(baseAUDist[i].doubleValue())));
      }
      stringBuilder.append(" System 2 distances to center AU:\n");
      for (int i = 0; i < 10; i++) {
        stringBuilder.append(format(" %2d %2d\t%16.8f\n", i, targetAUDist[i].index(),
            sqrt(targetAUDist[i].doubleValue())));
      }
    }
    ...
    for (int i = 0; i < baseLength; i++) {
      ...
      for (int j = 0; j < targetLength; j++) {
        ...
        if (useSym && value < minZdiffs[currZ1][currZ2]) {
          ...
          if (logger.isLoggable(Level.FINEST)) {
            stringBuilder.append(
                format("\n \t Base sym op %2d (%3d):  \t\t\t  Target sym op %2d (%3d):", currZ1,
                    minZinds[currZ1][currZ2].sortedIndex(), currZ2,
                    minZinds[currZ1][currZ2].referenceIndex()));
            for (int k = 0; k < nAU * compareAtomsSize; k++) {
              ...
              stringBuilder.append(format(
                  ...
                  bXYZ[l], bXYZ[l + 1], bXYZ[l + 2], bXYZsym[l], bXYZsym[l + 1], bXYZsym[l + 2],
                  tXYZsym[l], tXYZsym[l + 1], tXYZsym[l + 2], tXYZ[l], tXYZ[l + 1], tXYZ[l + 2]));
            }
          }
          if (logger.isLoggable(Level.FINEST)) {
          ...
          if (logger.isLoggable(Level.FINER)) {
            ...
            if (logger.isLoggable(Level.FINEST)) {
              stringBuilder.append(
                  format(" %35s to %30s compare %30s", "Base transform", "overlap Coords:",
              ...
              for (int k = 0; k < compareAtomsSize; k++) {
                ...
                stringBuilder.append(format(
                    "\n from %9.3f %9.3f %9.3f to %9.3f %9.3f %9.3f compare %9.3f %9.3f %9.3f from %9.3f %9.3f %9.3f",
                    bXYZ[l], bXYZ[l + 1], bXYZ[l + 2], baseXYZ[l], baseXYZ[l + 1], baseXYZ[l + 2],
                    targetXYZ[l], targetXYZ[l + 1], targetXYZ[l + 2], tXYZ[l], tXYZ[l + 1],
                    tXYZ[l + 2]));
              }
            }
          }
    ...
    if (nAU == 1 && !strict) {
              ...
              if (logger.isLoggable(Level.INFO)) {
                if (printSym < value) {
                  ...
                  if (alchemicalAtoms == null) {
                    ...
                    alchemicalAtoms.append(format(" %3d %3d  ", i, j))
                        .append(i * compareAtomsSize + comparisonAtoms[k] + 1);
                    alchemicalAtoms2.append(format(" %3d %3d  ", i, j))
                        .append(j * compareAtomsSize + comparisonAtoms[k] + 1);
                  } else {
      ...
      if (useSave && !useSym) {
        if (machineLearning) {
          ...
          saveAssembly(file1, name1, bondList1, atoms1, forceField1,
              bestBaseNAUs[baseBestZ][targetBestZ], comparisonAtoms, nAU, "_c1", compNum, save);
          saveAssembly(file2, name2, bondList2, atoms2, forceField2,
              bestTargetNAUs[baseBestZ][targetBestZ], comparisonAtoms, nAU, "_c2", compNum, save);
        }
      }
    ...
    for (int l = 0; l < baseLength; l++) {
      ...
      prioritizeReplicates(baseXYZ, massN, massSum, baseCoM[0], compareAtomsSize, baseAUDist_2,
          baseInd, linkage);
      ...
      for (int i = 0; i < 4; i++) {
        for (int j = 0; j < nAU; j++) {
          int molIndex = baseAUDist_2[j].index() * nCoords;
      ...
      double maxDist = baseAUDist[baseAUDist.length - 1].doubleValue();
      if (logger.isLoggable(Level.FINEST)) {
        stringBuilder.append(format("\n System 1 farthest distance: %4.8f", sqrt(maxDist)));
      }
      for (int i = 0; i < nAU; i++) {
        int molIndex = baseAUDist_2[i].index() * nCoords;
      ...
      for (int m = 0; m < targetLength; m++) {
        ...
        if (strict || !reverse && baseTargetMap[m] == bIndex
          ...
          prioritizeReplicates(targetXYZ, massN, massSum, targetCoM, compareAtomsSize,
...
//            targetTransformSymOp[currZ1][currZ2] = targetTransformSymOp[currZ1][currZ2].append(new SymOp(rotation, Tr_0_0_0));
          ...
          if (logger.isLoggable(Level.FINE)) {
            ...
            for (int i = 0; i < nAU; i++) {
              ...
              int molIndex = pairedAUs[i].index() * nCoords;
...
//            targetTransformSymOp[currZ1][currZ2] = targetTransformSymOp[currZ1][currZ2].append(new SymOp(rotation, Tr_0_0_0));
          ...
          if (logger.isLoggable(Level.FINEST) && useSave) {
            saveAssembly(file1, name1, bondList1, atoms1, forceField1, baseNAUs[3], comparisonAtoms,
                nAU, "_c1_N", compNum, save);
            saveAssembly(file2, name2, bondList2, atoms2, forceField2, targetNAUs, comparisonAtoms,
                nAU, "_c2_N", compNum, save);
          }
          applyRotation(targetNAUs, rotation);
          applyRotation(targetXYZ, rotation);
          ...
          if (logger.isLoggable(Level.FINEST) && useSave) {
            saveAssembly(file1, name1, bondList1, atoms1, forceField1, baseNAUs[3], comparisonAtoms,
                nAU, "_c1_N2", compNum, save);
            saveAssembly(file2, name2, bondList2, atoms2, forceField2, targetNAUs, comparisonAtoms,
                nAU, "_c2_N2", compNum, save);
          }
    ...
    if (useSave) {
      if (machineLearning) {
        saveAssembly(file1, name1, bondList1, atoms1, forceField1,
            bestBaseNAUs[baseBestZ][targetBestZ], comparisonAtoms, nAU, "_c1", 0.000, compNum, save);
        saveAssembly(file2, name2, bondList2, atoms2, forceField2,
...
//      int[] baseLMN = determineExpansion(baseCrystal.getUnitCell(), reducedBaseCoords, comparisonAtoms, mass, nAU, z1, inflationFactor);
      ...
      prioritizeReplicates(baseXYZ, massN, massSum, baseCoM[0], compareAtomsSize, baseAUDist, 0,
          linkage);
...
//                mass2, nAU, z2, inflationFactor);
            ...
            if (logger.isLoggable(Level.FINEST) && save > 0) {
              saveAssembly(file1, name1, bondList1, atoms1, forceField1, baseXYZoriginal,
                  comparisonAtoms, nBaseCoords / 3, "_c1_rep", -1, save);
              saveAssembly(file2, name2, bondList2, atoms2, forceField2, targetXYZoriginal,
                  comparisonAtoms2, nTargetCoords / 3, "_c2_rep", -1, save);
            }
            ...
            rmsd = compare(file1, name1, bondList1, atoms1, forceField1, file2, name2, bondList2,
                ...
                targetSearchValue, matchTol, row * targetSize + column, strict, save,
                      ...
                      if (logger.isLoggable(Level.FINER)) {
                        ...
                        if (save > 0) {
                          saveAssembly(file2, name2, bondList2, atoms2, forceField2, symMol,
                              comparisonAtoms, 1, "_moved", 0, save);
                        }
  ...
  private static String asMatrixString(SymOp symOp) {
    ...
    return format(" %14.8f %14.8f %14.8f \\\n" + "           %14.8f %14.8f %14.8f \\\n"
            + "           %14.8f %14.8f %14.8f \\\n" + "           %14.8f %14.8f %14.8f ", values[0][0],
  ...
  private static boolean checkInflatedSphere(double[] xyz, double[] massN, double massSum,
    ...
    for (int i = 0; i < nAU; i++) {
      ...
      int[] lmn = symOps.get(indexOrder.get(auDist[i].index())).replicatesVector;
...
//        startLMN = determineExpansion(unitCell, reducedCoords, comparisonAtoms, mass, nAU, zPrime, inflationFactor);
      ...
      systemCoords = generateInflatedSphere(unitCell, reducedCoords, zPrime, nAU, linkage, mass, lmn,
          strict, symOps, indexOrder, inflationFactor);
    }
    return systemCoords;
  }
  ...
  private void pairEntities(int desiredAUs, int comparisonNum) {
    ...
    for (int i = 0; i < desiredAUs; i++) {
      ...
      Integer minIndex = -1;
      double[] baseCoMCurr = baseCoM[comparisonNum][baseAUDist_2[i].index()];
    ...
    if (linkage == 0) {
      ...
      for (int i = 0; i < nMols; i++) {
        ...
        for (int j = 0; j < nAtoms; j++) {
          ...
          double[] centerXYZ = {coordsXYZ[centerAtomIndex], coordsXYZ[centerAtomIndex + 1],
        ...
        for (int i = 0; i < numCheck; i++) {
          logger.finest(
              format("  AU %d Center: %3d Index: %3d Dist %4.4f CoM:(%6.4f %6.4f %6.4f)", i, index,
                  auDist[i].index(), sqrt(auDist[i].doubleValue()), targetMol[auDist[i].index()][0],
                  targetMol[auDist[i].index()][1], targetMol[auDist[i].index()][2]));
        }
        logger.finest("\n");
      }
    } else if (linkage == 1) {
      ...
      molDists2[0] = new DoubleIndexPair(auIndex0, auDist[0].doubleValue());
        ...
        for (int i = 0; i < numCheck; i++) {
          logger.finest(
              format("  1,2AU %d Center: %3d Index: %3d Dist %4.4f CoM:(%6.4f %6.4f %6.4f)", i,
                  index, molDists2[i].index(), sqrt(molDists2[i].doubleValue()),
                  targetMol[molDists2[i].index()][0], targetMol[molDists2[i].index()][1],
                  targetMol[molDists2[i].index()][2]));
        }
      }
      ...
      molDists3[0] = new DoubleIndexPair(molDists2[0].index(), molDists2[0].doubleValue());
      molDists3[1] = new DoubleIndexPair(molDists2[1].index(), molDists2[1].doubleValue());
      avgCenter = new double[3];
      int bound = min(3, molDists2.length);
      for (int i = 0; i < bound; i++) {
        int molIndex = molDists2[i].index();
      ...
      if (logger.isLoggable(Level.FINEST)) {
        ...
        centerOfMass(targetMol, coordsXYZ, mass, massSum, nAtoms);
        for (int i = 0; i < numCheck; i++) {
          auIndex0 = molDists3[i].index();
          logger.finest(
              format("  1,2,3AU %d Center: %3d Index: %3d Dist %4.4f CoM:(%6.4f %6.4f %6.4f)", i,
                  index, auIndex0, sqrt(molDists3[i].doubleValue()), targetMol[auIndex0][0],
      ...
      if (logger.isLoggable(Level.FINEST)) {
        ...
        centerOfMass(targetMol, coordsXYZ, mass, massSum, nAtoms);
        for (int i = 0; i < numCheck; i++) {
          int auIndex = auDist[i].index();
          logger.finest(
              format("  AU %d Center: %3d Index: %3d Dist %4.4f CoM:(%6.4f %6.4f %6.4f)", i, index,
                  auIndex, sqrt(auDist[i].doubleValue()), targetMol[auIndex][0],
  ...
  private RunningStatistics readMatrix(String filename) {
    ...
    RunningStatistics runningStatistics = distanceMatrixFilter.readDistanceMatrix(filename, baseSize,
        targetSize);
  ...
  private void saveAssembly(File file, String name, List<Bond> bondList, Atom[] atoms,
    ...
    // Save aperiodic system of the n_mol closest atoms for visualization.
