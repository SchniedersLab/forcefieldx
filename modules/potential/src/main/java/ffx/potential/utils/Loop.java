...
public class Loop {
  ...
  private double[] getSolutionCoordinates(int k, double[][][] rSolnN, double[][][] rSolnA,
    ...
    // Loop through again to build hydrogen and side-chains with current C,N,CA coordinates
    for (int i = startResidue + 1; i < endResidue - 1; i++) {
      ...
      switch (name) {
        ...
        case VAL -> {
          ...
          arraycopy(determineIntxyz(cb, dCG_CB, ca, dCG_CB_CA, n, rotScale * 180.0, 0), 0, cg1, 0,
              3); // CG1
          ...
          arraycopy(determineIntxyz(cg1, dHG_CG, cb, dHG_CG_CB, ca, 180.0, 0), 0, hg11, 0,
              3); // HG11
          coordsArray = fillCoordsArray(HG11, coordsArray, hg11);

          arraycopy(determineIntxyz(cg1, dHG_CG, cb, dHG_CG_CB, hg11, 109.4, 1), 0, hg12, 0,
              3); // HG12
          coordsArray = fillCoordsArray(HG12, coordsArray, hg12);

          arraycopy(determineIntxyz(cg1, dHG_CG, cb, dHG_CG_CB, hg11, 109.4, -1), 0, hg13, 0,
              3); // HG13
          coordsArray = fillCoordsArray(HG13, coordsArray, hg13);

          arraycopy(determineIntxyz(cg2, dHG_CG, cb, dHG_CG_CB, ca, 180.0, 0), 0, hg21, 0,
              3); // HG21
          coordsArray = fillCoordsArray(HG21, coordsArray, hg21);

          arraycopy(determineIntxyz(cg2, dHG_CG, cb, dHG_CG_CB, hg21, 109.4, 1), 0, hg22, 0,
              3); // HG22
          coordsArray = fillCoordsArray(HG22, coordsArray, hg22);

          arraycopy(determineIntxyz(cg2, dHG_CG, cb, dHG_CG_CB, hg21, 109.4, -1), 0, hg23, 0,
              3); // HG23
        ...
        case LEU -> {
          ...
          arraycopy(determineIntxyz(cb, dCG_CB, ca, dCG_CB_CA, n, rotScale * 180.0, 0), 0, cg, 0,
              3); // CG
          ...
          arraycopy(determineIntxyz(cd1, dHD_CD, cg, dHD_CD_CG, cb, 180.0, 0), 0, hd11, 0,
              3); // HD11
          coordsArray = fillCoordsArray(HD11, coordsArray, hd11);

          arraycopy(determineIntxyz(cd1, dHD_CD, cg, dHD_CD_CG, hd11, 109.4, 1), 0, hd12, 0,
              3); // HD12
          coordsArray = fillCoordsArray(HD12, coordsArray, hd12);

          arraycopy(determineIntxyz(cd1, dHD_CD, cg, dHD_CD_CG, hd11, 109.4, -1), 0, hd13, 0,
              3); // HD13
          coordsArray = fillCoordsArray(HD13, coordsArray, hd13);

          arraycopy(determineIntxyz(cd2, dHD_CD, cg, dHD_CD_CG, cb, 180.0, 0), 0, hd21, 0,
              3); // HD21
          coordsArray = fillCoordsArray(HD21, coordsArray, hd21);

          arraycopy(determineIntxyz(cd2, dHD_CD, cg, dHD_CD_CG, hd21, 109.4, 1), 0, hd22, 0,
              3); // HD22
          coordsArray = fillCoordsArray(HD22, coordsArray, hd22);

          arraycopy(determineIntxyz(cd2, dHD_CD, cg, dHD_CD_CG, hd21, 109.4, -1), 0, hd23, 0,
              3); // HD23
        ...
        case ILE -> {
          ...
          arraycopy(determineIntxyz(cb, dCG1_CB, ca, dCG1_CB_CA, n, rotScale * 180.0, 0), 0, cg1, 0,
              3); // CG1
          coordsArray = fillCoordsArray(CG1, coordsArray, cg1);

          arraycopy(determineIntxyz(cb, dCG2_CB, ca, dCG2_CB_CA, cg1, 109.5, 1), 0, cg2, 0,
              3); // CG2
          coordsArray = fillCoordsArray(CG2, coordsArray, cg2);

          arraycopy(determineIntxyz(cg1, dCD1_CG1, cb, dCD1_CG1_CB, ca, 180, 0), 0, cd1, 0,
              3); // CD1
          coordsArray = fillCoordsArray(CD1, coordsArray, cd1);
          ...
          arraycopy(determineIntxyz(cg1, dHG1_CG, cb, dHG1_CG_CB, cd1, 109.4, 1), 0, hg12, 0,
              3); // HG12
          coordsArray = fillCoordsArray(HG12, coordsArray, hg12);

          arraycopy(determineIntxyz(cg1, dHG1_CG, cb, dHG1_CG_CB, cd1, 109.4, -1), 0, hg13, 0,
              3); // HG13
          coordsArray = fillCoordsArray(HG13, coordsArray, hg13);

          arraycopy(determineIntxyz(cg2, dHG2_CG, cb, dHG2_CG_CB, cg1, 180.0, 0), 0, hg21, 0,
              3); // HG21
          coordsArray = fillCoordsArray(HG21, coordsArray, hg21);

          arraycopy(determineIntxyz(cg2, dHG2_CG, cb, dHG2_CG_CB, hg21, 109.0, 1), 0, hg22, 0,
              3); // HG22
          coordsArray = fillCoordsArray(HG22, coordsArray, hg22);

          arraycopy(determineIntxyz(cg2, dHG2_CG, cb, dHG2_CG_CB, hg21, 109.0, -1), 0, hg23, 0,
              3); // HG23
          coordsArray = fillCoordsArray(HG23, coordsArray, hg23);

          arraycopy(determineIntxyz(cd1, dHD_CD, cg1, dHD_CD1_CG1, cb, 180.0, 0), 0, hd11, 0,
              3); // HD11
          coordsArray = fillCoordsArray(HD11, coordsArray, hd11);

          arraycopy(determineIntxyz(cd1, dHD_CD, cg1, dHD_CD1_CG1, hd11, 109.0, 1), 0, hd12, 0,
              3); // HD12
          coordsArray = fillCoordsArray(HD12, coordsArray, hd12);

          arraycopy(determineIntxyz(cd1, dHD_CD, cg1, dHD_CD1_CG1, hd11, 109.0, -1), 0, hd13, 0,
              3); // HD13
          coordsArray = fillCoordsArray(HD13, coordsArray, hd13);
        }
        case SER -> {
          ...
          arraycopy(determineIntxyz(cb, dOG_CB, ca, dOG_CB_CA, n, rotScale * 180.0, 0), 0, og, 0,
              3); // OG
        ...
        case THR -> {
          ...
          arraycopy(determineIntxyz(cb, dOG1_CB, ca, dOG1_CB_CA, n, rotScale * 180.0, 0), 0, og1, 0,
              3); // OG1
          coordsArray = fillCoordsArray(OG1, coordsArray, og1);

          arraycopy(determineIntxyz(cb, dCG2_CB, ca, dCG2_CB_CA, og1, 107.7, 1), 0, cg2, 0,
              3); // CG2
          coordsArray = fillCoordsArray(CG2, coordsArray, cg2);
          ...
          arraycopy(determineIntxyz(og1, dHG1_OG1, cb, dHG1_OG1_CB, ca, 180.0, 0), 0, hg1, 0,
              3); // HG1
          coordsArray = fillCoordsArray(HG1, coordsArray, hg1);

          arraycopy(determineIntxyz(cg2, dHG2_CG2, cb, dHG2_CG2_CB, ca, 180.0, 0), 0, hg21, 0,
              3); // HG21
          coordsArray = fillCoordsArray(HG21, coordsArray, hg21);

          arraycopy(determineIntxyz(cg2, dHG2_CG2, cb, dHG2_CG2_CB, hg21, 109.0, 1), 0, hg22, 0,
              3); // HG22
          coordsArray = fillCoordsArray(HG22, coordsArray, hg22);

          arraycopy(determineIntxyz(cg2, dHG2_CG2, cb, dHG2_CG2_CB, hg21, 109.0, -1), 0, hg23, 0,
              3); // HG23
          coordsArray = fillCoordsArray(HG23, coordsArray, hg23);
        }
        case CYS, CYX -> {
          ...
          arraycopy(determineIntxyz(cb, dSG_CB, ca, dSG_CB_CA, n, rotScale * 180.0, 0), 0, sg, 0,
              3); // SG
        ...
        case PHE -> {
          ...
          arraycopy(determineIntxyz(cd1, dHD_CD, cg, dHD_CD1_CG, ce1, 120.0, 1), 0, hd1, 0,
              3); // HD1
          coordsArray = fillCoordsArray(HD1, coordsArray, hd1);

          arraycopy(determineIntxyz(cd2, dHD_CD, cg, dHD_CD1_CG, ce2, 120.0, 1), 0, hd2, 0,
              3); // HD2
        ...
        case TRP -> {
          ...
          arraycopy(determineIntxyz(cg, dCD2_CG, cb, dCD2_CG_CB, cd1, 108.0, 1), 0, cd2, 0,
              3); // CD2
          coordsArray = fillCoordsArray(CD2, coordsArray, cd2);

          arraycopy(determineIntxyz(cd1, dNE1_CD1, cg, dNE1_CD1_CG, cd2, 0.0, 0), 0, ne1, 0,
              3); // NE1
          coordsArray = fillCoordsArray(NE1, coordsArray, ne1);

          arraycopy(determineIntxyz(ne1, dCE2_NE1, cd1, dCE2_NE1_CD1, cg, 0.0, 0), 0, ce2, 0,
              3); // CE2
          coordsArray = fillCoordsArray(CE2, coordsArray, ce2);

          arraycopy(determineIntxyz(cd2, dCE3_CD2, ce2, dCE3_CD2_CE2, ne1, 180.0, 0), 0, ce3, 0,
              3); // CE3
          coordsArray = fillCoordsArray(CE3, coordsArray, ce3);

          arraycopy(determineIntxyz(ce2, dCZ2_CE2, cd2, dCZ2_CE2_CD2, ce3, 0.0, 0), 0, cz2, 0,
              3); // CZ2
          coordsArray = fillCoordsArray(CZ2, coordsArray, cz2);

          arraycopy(determineIntxyz(ce3, dCZ3_CE3, cd2, dCZ3_CE3_CD2, ce2, 0.0, 0), 0, cz3, 0,
              3); // CZ3
          coordsArray = fillCoordsArray(CZ3, coordsArray, cz3);

          arraycopy(determineIntxyz(cz2, dCH2_CZ2, ce2, dCH2_CZ2_CE2, cd2, 0.0, 0), 0, ch2, 0,
              3); // CH2
          ...
          arraycopy(determineIntxyz(cd1, dHD1_CD1, cg, dHD1_CD1_CG, ne1, 126.0, 1), 0, hd1, 0,
              3); // HD1
          coordsArray = fillCoordsArray(HD1, coordsArray, hd1);

          arraycopy(determineIntxyz(ne1, dHE1_NE1, cd1, dHE1_NE1_CD1, ce2, 126.0, 1), 0, he1, 0,
              3); // HE1
          coordsArray = fillCoordsArray(HE1, coordsArray, he1);

          arraycopy(determineIntxyz(ce3, dHE3_CE3, cd2, dHE3_CE3_CD2, cz3, 120.0, 1), 0, he3, 0,
              3); // HE3
          coordsArray = fillCoordsArray(HE3, coordsArray, he3);

          arraycopy(determineIntxyz(cz2, dHZ2_CZ2, ce2, dHZ2_CZ2_CE2, ch2, 120.0, 1), 0, hz2, 0,
              3); // HZ2
          coordsArray = fillCoordsArray(HZ2, coordsArray, hz2);

          arraycopy(determineIntxyz(cz3, dHZ3_CZ3, ce3, dHZ3_CZ3_CE3, ch2, 120.0, 1), 0, hz3, 0,
              3); // HZ3
          coordsArray = fillCoordsArray(HZ3, coordsArray, hz3);

          arraycopy(determineIntxyz(ch2, dHH2_CH2, cz2, dHH2_CH2_CZ2, cz3, 120.0, 1), 0, hh2, 0,
              3); // HH2
        ...
        case HIS -> {
          ...
          arraycopy(determineIntxyz(cg, dCD2_CG, cb, dCD2_CG_CB, nd1, 108.0, 1), 0, cd2, 0,
              3); // CD2
          coordsArray = fillCoordsArray(CD2, coordsArray, cd2);

          arraycopy(determineIntxyz(nd1, dCE1_ND1, cg, dCE1_ND1_CG, cd2, 0.0, 0), 0, ce1, 0,
              3); // CE1
          coordsArray = fillCoordsArray(CE1, coordsArray, ce1);

          arraycopy(determineIntxyz(cd2, dNE2_CD2, cg, dNE2_CD2_CG, nd1, 0.0, 0), 0, ne2, 0,
              3); // NE2
          ...
          arraycopy(determineIntxyz(nd1, dHD1_ND1, cg, dHD1_ND1_CG, cb, 0.0, 0), 0, hd1, 0,
              3); // HD1
          coordsArray = fillCoordsArray(HD1, coordsArray, hd1);

          arraycopy(determineIntxyz(cd2, dHD2_CD2, cg, dHD2_CD2_CG, ne2, 126.0, 1), 0, hd2, 0,
              3); // HD2
          coordsArray = fillCoordsArray(HD2, coordsArray, hd2);

          arraycopy(determineIntxyz(ce1, dHE1_CE1, nd1, dHE1_CE1_ND1, ne2, 126.0, 1), 0, he1, 0,
              3); // HE1
          coordsArray = fillCoordsArray(HE1, coordsArray, he1);

          arraycopy(determineIntxyz(ne2, dHE2_NE2, cd2, dHE2_NE2_CD2, ce1, 126.0, 1), 0, he2, 0,
              3); // HE2
          coordsArray = fillCoordsArray(HE2, coordsArray, he2);
        }
        case HID -> {
        ...
        case ASP -> {
          ...
          arraycopy(determineIntxyz(cg, dOD2_CG, cb, dOD2_CG_CB, od1, 126.0, 1), 0, od2, 0,
              3); // OD2
        ...
        case ASH -> {
          ...
          arraycopy(determineIntxyz(od2, dHD2_OD2, cg, dHD2_OD2_CG, od1, 0.0, 0), 0, hd2, 0,
              3); // HD2
          coordsArray = fillCoordsArray(HD2, coordsArray, hd2);
        }
        case ASN -> {
          ...
          arraycopy(determineIntxyz(cg, dND2_CG, cb, dND2_CG_CB, od1, 124.0, 1), 0, nd2, 0,
              3); // ND2
          ...
          arraycopy(determineIntxyz(nd2, dHD2_ND2, cg, dHD2_ND2_CG, cb, 0.0, 0), 0, hd21, 0,
              3); // HD21
          coordsArray = fillCoordsArray(HD21, coordsArray, hd21);

          arraycopy(determineIntxyz(nd2, dHD2_ND2, cg, dHD2_ND2_CG, hd21, 120.0, 1), 0, hd22, 0,
              3); // HD22
          coordsArray = fillCoordsArray(HD22, coordsArray, hd22);
        }
        case GLU -> {
          ...
          arraycopy(determineIntxyz(cd, dOE2_CD, cg, dOE2_CD_CG, oe1, 126.0, 1), 0, oe2, 0,
              3); // OE2
        ...
        case GLH -> {
          ...
          arraycopy(determineIntxyz(oe2, dHE2_OE2, cd, dHE2_OE2_CD, oe1, 0.0, 0), 0, he2, 0,
              3); // HE2
          coordsArray = fillCoordsArray(HE2, coordsArray, he2);
        }
        case GLN -> {
          ...
          arraycopy(determineIntxyz(cd, dNE2_CD, cg, dNE2_CD_CG, oe1, 124.0, 1), 0, ne2, 0,
              3); // NE2
          coordsArray = fillCoordsArray(NE2, coordsArray, ne2);
          ...
          arraycopy(determineIntxyz(ne2, dHE2_NE2, cd, dHE2_NE2_CD, cg, 0.0, 0), 0, he21, 0,
              3); // HE21
          coordsArray = fillCoordsArray(HE21, coordsArray, he21);

          arraycopy(determineIntxyz(ne2, dHE2_NE2, cd, dHE2_NE2_CD, he21, 120.0, 1), 0, he22, 0,
              3); // HE22
        ...
        case ARG -> {
          ...
          arraycopy(determineIntxyz(nh1, dHH_NH, cz, dHH_NH_CZ, ne, 180.0, 0), 0, hh11, 0,
              3); // HH11
          coordsArray = fillCoordsArray(HH11, coordsArray, hh11);

          arraycopy(determineIntxyz(nh1, dHH_NH, cz, dHH_NH_CZ, hh11, 120.0, 1), 0, hh12, 0,
              3); // HH12
          coordsArray = fillCoordsArray(HH12, coordsArray, hh12);

          arraycopy(determineIntxyz(nh2, dHH_NH, cz, dHH_NH_CZ, ne, 180.0, 0), 0, hh21, 0,
              3); // HH21
          coordsArray = fillCoordsArray(HH21, coordsArray, hh21);

          arraycopy(determineIntxyz(nh2, dHH_NH, cz, dHH_NH_CZ, hh21, 120.0, 1), 0, hh22, 0,
              3); // HH22
